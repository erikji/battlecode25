package SPAARK;

import battlecode.common.*;

public class Splasher {
    public static final int EXPLORE = 0;
    public static final int ATTACK = 1;
    public static final int RETREAT = 2;
    public static int mode = EXPLORE;
    // controls round between visiting ruins
    public static final int VISIT_TIMEOUT = 75;

    public static MapLocation attackTarget = new MapLocation(-1, -1);
    public static int attackTargetTower;
    public static StringBuilder triedAttackTargets = new StringBuilder();

    public static int[] moveScores = new int[9];
    public static int[] attackScores = new int[37]; //score for attacking this square

    /**
     * Always:
     * If low on paint, retreat
     * Default to explore mode
     * If found targets to paint in POI storage, switch to attack mode
     * 
     * Explore:
     * Run around randomly painting stuff with some microstrategy
     * TODO: AVOID PAINTING OVER SRP, TOWER PATTERNS, PAINT NEAR OWN PAINT TO AVOID
     * TODO: DETACHED TERRITORY
     * 
     * Attack:
     * Go to targeted POI location and throw paint everywhere
     * Find new target if there's no threat
     */
    public static void run() throws Exception {
        // occasionally clear ruins to not oof forever
        if (G.rc.getPaint() < Robot.getRetreatPaint()) {
            mode = RETREAT;
        } else if (G.rc.getPaint() > G.rc.getType().paintCapacity * 3 / 4 && mode == RETREAT) {
            mode = EXPLORE;
        }
        if (mode != RETREAT) {
            updateAttackTarget();
        }
        else {
            triedAttackTargets = new StringBuilder();
        }
        // int a = Clock.getBytecodeNum();
        // switch (mode) {
        // ADD CASES HERE FOR SWITCHING MODES
        // }
        int b = Clock.getBytecodeNum();
        // G.indicatorString.append((b - a) + " ");
        attackScores[0] = attackScores[1] = attackScores[2] = attackScores[3] = attackScores[4] = attackScores[5] = attackScores[6] = attackScores[7] = attackScores[8] = attackScores[9] = attackScores[10] = attackScores[11] = attackScores[12] = attackScores[13] = attackScores[14] = attackScores[15] = attackScores[16] = attackScores[17] = attackScores[18] = attackScores[19] = attackScores[20] = attackScores[21] = attackScores[22] = attackScores[23] = attackScores[24] = attackScores[25] = attackScores[26] = attackScores[27] = attackScores[28] = attackScores[29] = attackScores[30] = attackScores[31] = attackScores[32] = attackScores[33] = attackScores[34] = attackScores[35] = attackScores[36] = moveScores[0] = moveScores[1] = moveScores[2] = moveScores[3] = moveScores[4] = moveScores[5] = moveScores[6] = moveScores[7] = moveScores[8] = 0;
        switch (mode) {
            case EXPLORE -> {
				G.indicatorString.append("EXPLORE ");
				if (G.rc.isMovementReady()) {
					exploreMoveScores();
				}
				if (G.rc.isActionReady()) {
                    //org.objectweb.asm.MethodTooLargeException: Method too large: SPAARK/Splasher.exploreAttackScores
					exploreAttackScores1();
					exploreAttackScores2();
				}
				G.rc.setIndicatorDot(G.me, 0, 255, 0);
			}
            case ATTACK -> {
				G.indicatorString.append("ATTACK ");
				if (G.rc.isMovementReady()) {
					attackMoveScores();
				}
				if (G.rc.isActionReady()) {
					attackAttackScores();
				}
				G.rc.setIndicatorDot(G.me, 0, 0, 255);
                G.rc.setIndicatorLine(G.me, attackTarget, 0, 0, 255);
			}
            case RETREAT -> {
                G.indicatorString.append("RETREAT ");
                Robot.retreat();
            }
        }
        int cmax = attackScores[0];
        int cx = 0;
        int cy = 0;
        if (attackScores[1] > cmax) {
            cmax = attackScores[1];
            cx = -1;
            cy = 0;
        }
        if (attackScores[2] > cmax) {
            cmax = attackScores[2];
            cx = 0;
            cy = -1;
        }
        if (attackScores[3] > cmax) {
            cmax = attackScores[3];
            cx = 0;
            cy = 1;
        }
        if (attackScores[4] > cmax) {
            cmax = attackScores[4];
            cx = 1;
            cy = 0;
        }
        if (attackScores[5] > cmax) {
            cmax = attackScores[5];
            cx = -1;
            cy = -1;
        }
        if (attackScores[6] > cmax) {
            cmax = attackScores[6];
            cx = -1;
            cy = 1;
        }
        if (attackScores[7] > cmax) {
            cmax = attackScores[7];
            cx = 1;
            cy = -1;
        }
        if (attackScores[8] > cmax) {
            cmax = attackScores[8];
            cx = 1;
            cy = 1;
        }
        // store total score and best attack location for each direction (incl
        // Direction.CENTER)
        int[] allmax = new int[] {
            cmax, cmax, cmax, cmax, cmax, cmax, cmax, cmax, cmax
        };
        int[] allx = new int[] {
                cx, cx, cx, cx, cx, cx, cx, cx, cx
        };
        int[] ally = new int[] {
                cy, cy, cy, cy, cy, cy, cy, cy, cy
        };
        if (attackScores[13] > allmax[0]) {
			allmax[0] = attackScores[13];
			allx[0] = -2;
			ally[0] = -1;
		}
		if (attackScores[15] > allmax[0]) {
			allmax[0] = attackScores[15];
			allx[0] = -1;
			ally[0] = -2;
		}
		if (attackScores[21] > allmax[0]) {
			allmax[0] = attackScores[21];
			allx[0] = -2;
			ally[0] = -2;
		}
		if (attackScores[29] > allmax[0]) {
			allmax[0] = attackScores[29];
			allx[0] = -3;
			ally[0] = -1;
		}
		if (attackScores[31] > allmax[0]) {
			allmax[0] = attackScores[31];
			allx[0] = -1;
			ally[0] = -3;
		}
		if (attackScores[15] > allmax[1]) {
			allmax[1] = attackScores[15];
			allx[1] = -1;
			ally[1] = -2;
		}
		if (attackScores[17] > allmax[1]) {
			allmax[1] = attackScores[17];
			allx[1] = 1;
			ally[1] = -2;
		}
		if (attackScores[13] > allmax[1]) {
			allmax[1] = attackScores[13];
			allx[1] = -2;
			ally[1] = -1;
		}
		if (attackScores[26] > allmax[1]) {
			allmax[1] = attackScores[26];
			allx[1] = 0;
			ally[1] = -3;
		}
		if (attackScores[19] > allmax[1]) {
			allmax[1] = attackScores[19];
			allx[1] = 2;
			ally[1] = -1;
		}
		if (attackScores[17] > allmax[2]) {
			allmax[2] = attackScores[17];
			allx[2] = 1;
			ally[2] = -2;
		}
		if (attackScores[19] > allmax[2]) {
			allmax[2] = attackScores[19];
			allx[2] = 2;
			ally[2] = -1;
		}
		if (attackScores[23] > allmax[2]) {
			allmax[2] = attackScores[23];
			allx[2] = 2;
			ally[2] = -2;
		}
		if (attackScores[33] > allmax[2]) {
			allmax[2] = attackScores[33];
			allx[2] = 1;
			ally[2] = -3;
		}
		if (attackScores[35] > allmax[2]) {
			allmax[2] = attackScores[35];
			allx[2] = 3;
			ally[2] = -1;
		}
		if (attackScores[19] > allmax[3]) {
			allmax[3] = attackScores[19];
			allx[3] = 2;
			ally[3] = -1;
		}
		if (attackScores[20] > allmax[3]) {
			allmax[3] = attackScores[20];
			allx[3] = 2;
			ally[3] = 1;
		}
		if (attackScores[17] > allmax[3]) {
			allmax[3] = attackScores[17];
			allx[3] = 1;
			ally[3] = -2;
		}
		if (attackScores[18] > allmax[3]) {
			allmax[3] = attackScores[18];
			allx[3] = 1;
			ally[3] = 2;
		}
		if (attackScores[28] > allmax[3]) {
			allmax[3] = attackScores[28];
			allx[3] = 3;
			ally[3] = 0;
		}
		if (attackScores[18] > allmax[4]) {
			allmax[4] = attackScores[18];
			allx[4] = 1;
			ally[4] = 2;
		}
		if (attackScores[20] > allmax[4]) {
			allmax[4] = attackScores[20];
			allx[4] = 2;
			ally[4] = 1;
		}
		if (attackScores[24] > allmax[4]) {
			allmax[4] = attackScores[24];
			allx[4] = 2;
			ally[4] = 2;
		}
		if (attackScores[34] > allmax[4]) {
			allmax[4] = attackScores[34];
			allx[4] = 1;
			ally[4] = 3;
		}
		if (attackScores[36] > allmax[4]) {
			allmax[4] = attackScores[36];
			allx[4] = 3;
			ally[4] = 1;
		}
		if (attackScores[16] > allmax[5]) {
			allmax[5] = attackScores[16];
			allx[5] = -1;
			ally[5] = 2;
		}
		if (attackScores[18] > allmax[5]) {
			allmax[5] = attackScores[18];
			allx[5] = 1;
			ally[5] = 2;
		}
		if (attackScores[14] > allmax[5]) {
			allmax[5] = attackScores[14];
			allx[5] = -2;
			ally[5] = 1;
		}
		if (attackScores[27] > allmax[5]) {
			allmax[5] = attackScores[27];
			allx[5] = 0;
			ally[5] = 3;
		}
		if (attackScores[20] > allmax[5]) {
			allmax[5] = attackScores[20];
			allx[5] = 2;
			ally[5] = 1;
		}
		if (attackScores[14] > allmax[6]) {
			allmax[6] = attackScores[14];
			allx[6] = -2;
			ally[6] = 1;
		}
		if (attackScores[16] > allmax[6]) {
			allmax[6] = attackScores[16];
			allx[6] = -1;
			ally[6] = 2;
		}
		if (attackScores[22] > allmax[6]) {
			allmax[6] = attackScores[22];
			allx[6] = -2;
			ally[6] = 2;
		}
		if (attackScores[30] > allmax[6]) {
			allmax[6] = attackScores[30];
			allx[6] = -3;
			ally[6] = 1;
		}
		if (attackScores[32] > allmax[6]) {
			allmax[6] = attackScores[32];
			allx[6] = -1;
			ally[6] = 3;
		}
		if (attackScores[13] > allmax[7]) {
			allmax[7] = attackScores[13];
			allx[7] = -2;
			ally[7] = -1;
		}
		if (attackScores[14] > allmax[7]) {
			allmax[7] = attackScores[14];
			allx[7] = -2;
			ally[7] = 1;
		}
		if (attackScores[25] > allmax[7]) {
			allmax[7] = attackScores[25];
			allx[7] = -3;
			ally[7] = 0;
		}
		if (attackScores[15] > allmax[7]) {
			allmax[7] = attackScores[15];
			allx[7] = -1;
			ally[7] = -2;
		}
		if (attackScores[16] > allmax[7]) {
			allmax[7] = attackScores[16];
			allx[7] = -1;
			ally[7] = 2;
		}
        // copyspaghetti from Motion.microMove and Mopper but whatever
        boolean notMoved = true;
        if (G.rc.isActionReady()) {
            int best = 8;
            int numBest = 1;
            for (int i = 8; --i >= 0;) {
                if (allmax[i] + moveScores[i] > allmax[best] + moveScores[best]) {
                    best = i;
                    numBest = 1;
                } else if (allmax[i] + moveScores[i] == allmax[best] + moveScores[best] && Random.rand() % ++numBest == 0) {
                    best = i;
                }
            }
            if (allmax[best] > 250) {
                MapLocation attackLoc = G.me.translate(allx[best], ally[best]);
				//try to move before attacking if possible so the paint we used in the attack isn't factored into movement cooldown
				if (attackLoc.isWithinDistanceSquared(G.me.add(G.ALL_DIRECTIONS[best]), 4)) {
					Motion.move(G.ALL_DIRECTIONS[best]);
					if (G.rc.canAttack(attackLoc)) {
						G.rc.attack(attackLoc);
					}
				} else {
					if (G.rc.canAttack(attackLoc)) {
						G.rc.attack(attackLoc);
					}
					Motion.move(G.ALL_DIRECTIONS[best]);
				}
                notMoved = true;
            }
        }
        if (notMoved) {
            int best = 8;
            int numBest = 1;
            for (int i = 8; --i >= 0;) {
                if (moveScores[i] > moveScores[best]) {
                    best = i;
                    numBest = 1;
                } else if (moveScores[i] == moveScores[best] && Random.rand() % ++numBest == 0) {
                    best = i;
                }
            }
            Motion.move(G.ALL_DIRECTIONS[best]);
        }
        G.indicatorString.append((Clock.getBytecodeNum() - b) + " ");
    }

    public static void exploreAttackScores1() throws Exception {
        MapLocation[] ruins = G.rc.senseNearbyRuins(-1);
		StringBuilder ruinsString = new StringBuilder();
		for (int i = ruins.length; --i >= 0;) {
			ruinsString.append(ruins[i].toString());
		}
		MapLocation loc = G.me;
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[12] += 25;
				attackScores[11] += 25;
				attackScores[9] += 25;
				attackScores[10] += 25;
				attackScores[5] += 25;
				attackScores[2] += 25;
				attackScores[7] += 25;
				attackScores[4] += 25;
				attackScores[8] += 25;
				attackScores[3] += 25;
				attackScores[6] += 25;
				attackScores[1] += 25;
				attackScores[0] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[12] += 25;
						attackScores[11] += 25;
						attackScores[9] += 25;
						attackScores[10] += 25;
						attackScores[5] += 25;
						attackScores[2] += 25;
						attackScores[7] += 25;
						attackScores[4] += 25;
						attackScores[8] += 25;
						attackScores[3] += 25;
						attackScores[6] += 25;
						attackScores[1] += 25;
						attackScores[0] += 25;
					} else {
						attackScores[12] += G.paintPerChips() * 500;
						attackScores[11] += G.paintPerChips() * 500;
						attackScores[9] += G.paintPerChips() * 500;
						attackScores[10] += G.paintPerChips() * 500;
						attackScores[5] += G.paintPerChips() * 500;
						attackScores[2] += G.paintPerChips() * 500;
						attackScores[7] += G.paintPerChips() * 500;
						attackScores[4] += G.paintPerChips() * 500;
						attackScores[8] += G.paintPerChips() * 500;
						attackScores[3] += G.paintPerChips() * 500;
						attackScores[6] += G.paintPerChips() * 500;
						attackScores[1] += G.paintPerChips() * 500;
						attackScores[0] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[12] += 25;
					attackScores[11] += 25;
					attackScores[9] += 25;
					attackScores[10] += 25;
					attackScores[5] += 25;
					attackScores[2] += 25;
					attackScores[7] += 25;
					attackScores[4] += 25;
					attackScores[8] += 25;
					attackScores[3] += 25;
					attackScores[6] += 25;
					attackScores[1] += 25;
					attackScores[0] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[5] += 50;
				attackScores[2] += 50;
				attackScores[7] += 50;
				attackScores[4] += 50;
				attackScores[8] += 50;
				attackScores[3] += 50;
				attackScores[6] += 50;
				attackScores[1] += 50;
				attackScores[0] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[12] += 50;
					attackScores[11] += 50;
					attackScores[9] += 50;
					attackScores[10] += 50;
					attackScores[5] += 50;
					attackScores[2] += 50;
					attackScores[7] += 50;
					attackScores[4] += 50;
					attackScores[8] += 50;
					attackScores[3] += 50;
					attackScores[6] += 50;
					attackScores[1] += 50;
					attackScores[0] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[12] += 50;
					attackScores[11] += 50;
					attackScores[9] += 50;
					attackScores[10] += 50;
					attackScores[5] += 50;
					attackScores[2] += 50;
					attackScores[7] += 50;
					attackScores[4] += 50;
					attackScores[8] += 50;
					attackScores[3] += 50;
					attackScores[6] += 50;
					attackScores[1] += 50;
					attackScores[0] += 50;
				}
			}
		}
		loc = G.me.translate(-1, 0);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[4] += 25;
				attackScores[16] += 25;
				attackScores[25] += 25;
				attackScores[15] += 25;
				attackScores[13] += 25;
				attackScores[5] += 25;
				attackScores[2] += 25;
				attackScores[0] += 25;
				attackScores[3] += 25;
				attackScores[6] += 25;
				attackScores[14] += 25;
				attackScores[9] += 25;
				attackScores[1] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[4] += 25;
						attackScores[16] += 25;
						attackScores[25] += 25;
						attackScores[15] += 25;
						attackScores[13] += 25;
						attackScores[5] += 25;
						attackScores[2] += 25;
						attackScores[0] += 25;
						attackScores[3] += 25;
						attackScores[6] += 25;
						attackScores[14] += 25;
						attackScores[9] += 25;
						attackScores[1] += 25;
					} else {
						attackScores[4] += G.paintPerChips() * 500;
						attackScores[16] += G.paintPerChips() * 500;
						attackScores[25] += G.paintPerChips() * 500;
						attackScores[15] += G.paintPerChips() * 500;
						attackScores[13] += G.paintPerChips() * 500;
						attackScores[5] += G.paintPerChips() * 500;
						attackScores[2] += G.paintPerChips() * 500;
						attackScores[0] += G.paintPerChips() * 500;
						attackScores[3] += G.paintPerChips() * 500;
						attackScores[6] += G.paintPerChips() * 500;
						attackScores[14] += G.paintPerChips() * 500;
						attackScores[9] += G.paintPerChips() * 500;
						attackScores[1] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[4] += 25;
					attackScores[16] += 25;
					attackScores[25] += 25;
					attackScores[15] += 25;
					attackScores[13] += 25;
					attackScores[5] += 25;
					attackScores[2] += 25;
					attackScores[0] += 25;
					attackScores[3] += 25;
					attackScores[6] += 25;
					attackScores[14] += 25;
					attackScores[9] += 25;
					attackScores[1] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[13] += 50;
				attackScores[5] += 50;
				attackScores[2] += 50;
				attackScores[0] += 50;
				attackScores[3] += 50;
				attackScores[6] += 50;
				attackScores[14] += 50;
				attackScores[9] += 50;
				attackScores[1] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[4] += 50;
					attackScores[16] += 50;
					attackScores[25] += 50;
					attackScores[15] += 50;
					attackScores[13] += 50;
					attackScores[5] += 50;
					attackScores[2] += 50;
					attackScores[0] += 50;
					attackScores[3] += 50;
					attackScores[6] += 50;
					attackScores[14] += 50;
					attackScores[9] += 50;
					attackScores[1] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[4] += 50;
					attackScores[16] += 50;
					attackScores[25] += 50;
					attackScores[15] += 50;
					attackScores[13] += 50;
					attackScores[5] += 50;
					attackScores[2] += 50;
					attackScores[0] += 50;
					attackScores[3] += 50;
					attackScores[6] += 50;
					attackScores[14] += 50;
					attackScores[9] += 50;
					attackScores[1] += 50;
				}
			}
		}
		loc = G.me.translate(0, -1);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[19] += 25;
				attackScores[3] += 25;
				attackScores[13] += 25;
				attackScores[26] += 25;
				attackScores[15] += 25;
				attackScores[10] += 25;
				attackScores[17] += 25;
				attackScores[7] += 25;
				attackScores[4] += 25;
				attackScores[0] += 25;
				attackScores[1] += 25;
				attackScores[5] += 25;
				attackScores[2] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[19] += 25;
						attackScores[3] += 25;
						attackScores[13] += 25;
						attackScores[26] += 25;
						attackScores[15] += 25;
						attackScores[10] += 25;
						attackScores[17] += 25;
						attackScores[7] += 25;
						attackScores[4] += 25;
						attackScores[0] += 25;
						attackScores[1] += 25;
						attackScores[5] += 25;
						attackScores[2] += 25;
					} else {
						attackScores[19] += G.paintPerChips() * 500;
						attackScores[3] += G.paintPerChips() * 500;
						attackScores[13] += G.paintPerChips() * 500;
						attackScores[26] += G.paintPerChips() * 500;
						attackScores[15] += G.paintPerChips() * 500;
						attackScores[10] += G.paintPerChips() * 500;
						attackScores[17] += G.paintPerChips() * 500;
						attackScores[7] += G.paintPerChips() * 500;
						attackScores[4] += G.paintPerChips() * 500;
						attackScores[0] += G.paintPerChips() * 500;
						attackScores[1] += G.paintPerChips() * 500;
						attackScores[5] += G.paintPerChips() * 500;
						attackScores[2] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[19] += 25;
					attackScores[3] += 25;
					attackScores[13] += 25;
					attackScores[26] += 25;
					attackScores[15] += 25;
					attackScores[10] += 25;
					attackScores[17] += 25;
					attackScores[7] += 25;
					attackScores[4] += 25;
					attackScores[0] += 25;
					attackScores[1] += 25;
					attackScores[5] += 25;
					attackScores[2] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[15] += 50;
				attackScores[10] += 50;
				attackScores[17] += 50;
				attackScores[7] += 50;
				attackScores[4] += 50;
				attackScores[0] += 50;
				attackScores[1] += 50;
				attackScores[5] += 50;
				attackScores[2] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[19] += 50;
					attackScores[3] += 50;
					attackScores[13] += 50;
					attackScores[26] += 50;
					attackScores[15] += 50;
					attackScores[10] += 50;
					attackScores[17] += 50;
					attackScores[7] += 50;
					attackScores[4] += 50;
					attackScores[0] += 50;
					attackScores[1] += 50;
					attackScores[5] += 50;
					attackScores[2] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[19] += 50;
					attackScores[3] += 50;
					attackScores[13] += 50;
					attackScores[26] += 50;
					attackScores[15] += 50;
					attackScores[10] += 50;
					attackScores[17] += 50;
					attackScores[7] += 50;
					attackScores[4] += 50;
					attackScores[0] += 50;
					attackScores[1] += 50;
					attackScores[5] += 50;
					attackScores[2] += 50;
				}
			}
		}
		loc = G.me.translate(0, 1);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[20] += 25;
				attackScores[27] += 25;
				attackScores[14] += 25;
				attackScores[2] += 25;
				attackScores[1] += 25;
				attackScores[0] += 25;
				attackScores[4] += 25;
				attackScores[8] += 25;
				attackScores[18] += 25;
				attackScores[11] += 25;
				attackScores[16] += 25;
				attackScores[6] += 25;
				attackScores[3] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[20] += 25;
						attackScores[27] += 25;
						attackScores[14] += 25;
						attackScores[2] += 25;
						attackScores[1] += 25;
						attackScores[0] += 25;
						attackScores[4] += 25;
						attackScores[8] += 25;
						attackScores[18] += 25;
						attackScores[11] += 25;
						attackScores[16] += 25;
						attackScores[6] += 25;
						attackScores[3] += 25;
					} else {
						attackScores[20] += G.paintPerChips() * 500;
						attackScores[27] += G.paintPerChips() * 500;
						attackScores[14] += G.paintPerChips() * 500;
						attackScores[2] += G.paintPerChips() * 500;
						attackScores[1] += G.paintPerChips() * 500;
						attackScores[0] += G.paintPerChips() * 500;
						attackScores[4] += G.paintPerChips() * 500;
						attackScores[8] += G.paintPerChips() * 500;
						attackScores[18] += G.paintPerChips() * 500;
						attackScores[11] += G.paintPerChips() * 500;
						attackScores[16] += G.paintPerChips() * 500;
						attackScores[6] += G.paintPerChips() * 500;
						attackScores[3] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[20] += 25;
					attackScores[27] += 25;
					attackScores[14] += 25;
					attackScores[2] += 25;
					attackScores[1] += 25;
					attackScores[0] += 25;
					attackScores[4] += 25;
					attackScores[8] += 25;
					attackScores[18] += 25;
					attackScores[11] += 25;
					attackScores[16] += 25;
					attackScores[6] += 25;
					attackScores[3] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[1] += 50;
				attackScores[0] += 50;
				attackScores[4] += 50;
				attackScores[8] += 50;
				attackScores[18] += 50;
				attackScores[11] += 50;
				attackScores[16] += 50;
				attackScores[6] += 50;
				attackScores[3] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[20] += 50;
					attackScores[27] += 50;
					attackScores[14] += 50;
					attackScores[2] += 50;
					attackScores[1] += 50;
					attackScores[0] += 50;
					attackScores[4] += 50;
					attackScores[8] += 50;
					attackScores[18] += 50;
					attackScores[11] += 50;
					attackScores[16] += 50;
					attackScores[6] += 50;
					attackScores[3] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[20] += 50;
					attackScores[27] += 50;
					attackScores[14] += 50;
					attackScores[2] += 50;
					attackScores[1] += 50;
					attackScores[0] += 50;
					attackScores[4] += 50;
					attackScores[8] += 50;
					attackScores[18] += 50;
					attackScores[11] += 50;
					attackScores[16] += 50;
					attackScores[6] += 50;
					attackScores[3] += 50;
				}
			}
		}
		loc = G.me.translate(1, 0);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[28] += 25;
				attackScores[18] += 25;
				attackScores[1] += 25;
				attackScores[17] += 25;
				attackScores[2] += 25;
				attackScores[7] += 25;
				attackScores[19] += 25;
				attackScores[12] += 25;
				attackScores[20] += 25;
				attackScores[8] += 25;
				attackScores[3] += 25;
				attackScores[0] += 25;
				attackScores[4] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[28] += 25;
						attackScores[18] += 25;
						attackScores[1] += 25;
						attackScores[17] += 25;
						attackScores[2] += 25;
						attackScores[7] += 25;
						attackScores[19] += 25;
						attackScores[12] += 25;
						attackScores[20] += 25;
						attackScores[8] += 25;
						attackScores[3] += 25;
						attackScores[0] += 25;
						attackScores[4] += 25;
					} else {
						attackScores[28] += G.paintPerChips() * 500;
						attackScores[18] += G.paintPerChips() * 500;
						attackScores[1] += G.paintPerChips() * 500;
						attackScores[17] += G.paintPerChips() * 500;
						attackScores[2] += G.paintPerChips() * 500;
						attackScores[7] += G.paintPerChips() * 500;
						attackScores[19] += G.paintPerChips() * 500;
						attackScores[12] += G.paintPerChips() * 500;
						attackScores[20] += G.paintPerChips() * 500;
						attackScores[8] += G.paintPerChips() * 500;
						attackScores[3] += G.paintPerChips() * 500;
						attackScores[0] += G.paintPerChips() * 500;
						attackScores[4] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[28] += 25;
					attackScores[18] += 25;
					attackScores[1] += 25;
					attackScores[17] += 25;
					attackScores[2] += 25;
					attackScores[7] += 25;
					attackScores[19] += 25;
					attackScores[12] += 25;
					attackScores[20] += 25;
					attackScores[8] += 25;
					attackScores[3] += 25;
					attackScores[0] += 25;
					attackScores[4] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[2] += 50;
				attackScores[7] += 50;
				attackScores[19] += 50;
				attackScores[12] += 50;
				attackScores[20] += 50;
				attackScores[8] += 50;
				attackScores[3] += 50;
				attackScores[0] += 50;
				attackScores[4] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[28] += 50;
					attackScores[18] += 50;
					attackScores[1] += 50;
					attackScores[17] += 50;
					attackScores[2] += 50;
					attackScores[7] += 50;
					attackScores[19] += 50;
					attackScores[12] += 50;
					attackScores[20] += 50;
					attackScores[8] += 50;
					attackScores[3] += 50;
					attackScores[0] += 50;
					attackScores[4] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[28] += 50;
					attackScores[18] += 50;
					attackScores[1] += 50;
					attackScores[17] += 50;
					attackScores[2] += 50;
					attackScores[7] += 50;
					attackScores[19] += 50;
					attackScores[12] += 50;
					attackScores[20] += 50;
					attackScores[8] += 50;
					attackScores[3] += 50;
					attackScores[0] += 50;
					attackScores[4] += 50;
				}
			}
		}
		loc = G.me.translate(-1, -1);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[7] += 25;
				attackScores[6] += 25;
				attackScores[29] += 25;
				attackScores[31] += 25;
				attackScores[21] += 25;
				attackScores[15] += 25;
				attackScores[10] += 25;
				attackScores[2] += 25;
				attackScores[0] += 25;
				attackScores[1] += 25;
				attackScores[9] += 25;
				attackScores[13] += 25;
				attackScores[5] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[7] += 25;
						attackScores[6] += 25;
						attackScores[29] += 25;
						attackScores[31] += 25;
						attackScores[21] += 25;
						attackScores[15] += 25;
						attackScores[10] += 25;
						attackScores[2] += 25;
						attackScores[0] += 25;
						attackScores[1] += 25;
						attackScores[9] += 25;
						attackScores[13] += 25;
						attackScores[5] += 25;
					} else {
						attackScores[7] += G.paintPerChips() * 500;
						attackScores[6] += G.paintPerChips() * 500;
						attackScores[29] += G.paintPerChips() * 500;
						attackScores[31] += G.paintPerChips() * 500;
						attackScores[21] += G.paintPerChips() * 500;
						attackScores[15] += G.paintPerChips() * 500;
						attackScores[10] += G.paintPerChips() * 500;
						attackScores[2] += G.paintPerChips() * 500;
						attackScores[0] += G.paintPerChips() * 500;
						attackScores[1] += G.paintPerChips() * 500;
						attackScores[9] += G.paintPerChips() * 500;
						attackScores[13] += G.paintPerChips() * 500;
						attackScores[5] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[7] += 25;
					attackScores[6] += 25;
					attackScores[29] += 25;
					attackScores[31] += 25;
					attackScores[21] += 25;
					attackScores[15] += 25;
					attackScores[10] += 25;
					attackScores[2] += 25;
					attackScores[0] += 25;
					attackScores[1] += 25;
					attackScores[9] += 25;
					attackScores[13] += 25;
					attackScores[5] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[21] += 50;
				attackScores[15] += 50;
				attackScores[10] += 50;
				attackScores[2] += 50;
				attackScores[0] += 50;
				attackScores[1] += 50;
				attackScores[9] += 50;
				attackScores[13] += 50;
				attackScores[5] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[7] += 50;
					attackScores[6] += 50;
					attackScores[29] += 50;
					attackScores[31] += 50;
					attackScores[21] += 50;
					attackScores[15] += 50;
					attackScores[10] += 50;
					attackScores[2] += 50;
					attackScores[0] += 50;
					attackScores[1] += 50;
					attackScores[9] += 50;
					attackScores[13] += 50;
					attackScores[5] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[7] += 50;
					attackScores[6] += 50;
					attackScores[29] += 50;
					attackScores[31] += 50;
					attackScores[21] += 50;
					attackScores[15] += 50;
					attackScores[10] += 50;
					attackScores[2] += 50;
					attackScores[0] += 50;
					attackScores[1] += 50;
					attackScores[9] += 50;
					attackScores[13] += 50;
					attackScores[5] += 50;
				}
			}
		}
		loc = G.me.translate(-1, 1);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[8] += 25;
				attackScores[32] += 25;
				attackScores[30] += 25;
				attackScores[5] += 25;
				attackScores[9] += 25;
				attackScores[1] += 25;
				attackScores[0] += 25;
				attackScores[3] += 25;
				attackScores[11] += 25;
				attackScores[16] += 25;
				attackScores[22] += 25;
				attackScores[14] += 25;
				attackScores[6] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[8] += 25;
						attackScores[32] += 25;
						attackScores[30] += 25;
						attackScores[5] += 25;
						attackScores[9] += 25;
						attackScores[1] += 25;
						attackScores[0] += 25;
						attackScores[3] += 25;
						attackScores[11] += 25;
						attackScores[16] += 25;
						attackScores[22] += 25;
						attackScores[14] += 25;
						attackScores[6] += 25;
					} else {
						attackScores[8] += G.paintPerChips() * 500;
						attackScores[32] += G.paintPerChips() * 500;
						attackScores[30] += G.paintPerChips() * 500;
						attackScores[5] += G.paintPerChips() * 500;
						attackScores[9] += G.paintPerChips() * 500;
						attackScores[1] += G.paintPerChips() * 500;
						attackScores[0] += G.paintPerChips() * 500;
						attackScores[3] += G.paintPerChips() * 500;
						attackScores[11] += G.paintPerChips() * 500;
						attackScores[16] += G.paintPerChips() * 500;
						attackScores[22] += G.paintPerChips() * 500;
						attackScores[14] += G.paintPerChips() * 500;
						attackScores[6] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[8] += 25;
					attackScores[32] += 25;
					attackScores[30] += 25;
					attackScores[5] += 25;
					attackScores[9] += 25;
					attackScores[1] += 25;
					attackScores[0] += 25;
					attackScores[3] += 25;
					attackScores[11] += 25;
					attackScores[16] += 25;
					attackScores[22] += 25;
					attackScores[14] += 25;
					attackScores[6] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[9] += 50;
				attackScores[1] += 50;
				attackScores[0] += 50;
				attackScores[3] += 50;
				attackScores[11] += 50;
				attackScores[16] += 50;
				attackScores[22] += 50;
				attackScores[14] += 50;
				attackScores[6] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[8] += 50;
					attackScores[32] += 50;
					attackScores[30] += 50;
					attackScores[5] += 50;
					attackScores[9] += 50;
					attackScores[1] += 50;
					attackScores[0] += 50;
					attackScores[3] += 50;
					attackScores[11] += 50;
					attackScores[16] += 50;
					attackScores[22] += 50;
					attackScores[14] += 50;
					attackScores[6] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[8] += 50;
					attackScores[32] += 50;
					attackScores[30] += 50;
					attackScores[5] += 50;
					attackScores[9] += 50;
					attackScores[1] += 50;
					attackScores[0] += 50;
					attackScores[3] += 50;
					attackScores[11] += 50;
					attackScores[16] += 50;
					attackScores[22] += 50;
					attackScores[14] += 50;
					attackScores[6] += 50;
				}
			}
		}
		loc = G.me.translate(1, -1);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[35] += 25;
				attackScores[8] += 25;
				attackScores[5] += 25;
				attackScores[33] += 25;
				attackScores[10] += 25;
				attackScores[17] += 25;
				attackScores[23] += 25;
				attackScores[19] += 25;
				attackScores[12] += 25;
				attackScores[4] += 25;
				attackScores[0] += 25;
				attackScores[2] += 25;
				attackScores[7] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[35] += 25;
						attackScores[8] += 25;
						attackScores[5] += 25;
						attackScores[33] += 25;
						attackScores[10] += 25;
						attackScores[17] += 25;
						attackScores[23] += 25;
						attackScores[19] += 25;
						attackScores[12] += 25;
						attackScores[4] += 25;
						attackScores[0] += 25;
						attackScores[2] += 25;
						attackScores[7] += 25;
					} else {
						attackScores[35] += G.paintPerChips() * 500;
						attackScores[8] += G.paintPerChips() * 500;
						attackScores[5] += G.paintPerChips() * 500;
						attackScores[33] += G.paintPerChips() * 500;
						attackScores[10] += G.paintPerChips() * 500;
						attackScores[17] += G.paintPerChips() * 500;
						attackScores[23] += G.paintPerChips() * 500;
						attackScores[19] += G.paintPerChips() * 500;
						attackScores[12] += G.paintPerChips() * 500;
						attackScores[4] += G.paintPerChips() * 500;
						attackScores[0] += G.paintPerChips() * 500;
						attackScores[2] += G.paintPerChips() * 500;
						attackScores[7] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[35] += 25;
					attackScores[8] += 25;
					attackScores[5] += 25;
					attackScores[33] += 25;
					attackScores[10] += 25;
					attackScores[17] += 25;
					attackScores[23] += 25;
					attackScores[19] += 25;
					attackScores[12] += 25;
					attackScores[4] += 25;
					attackScores[0] += 25;
					attackScores[2] += 25;
					attackScores[7] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[10] += 50;
				attackScores[17] += 50;
				attackScores[23] += 50;
				attackScores[19] += 50;
				attackScores[12] += 50;
				attackScores[4] += 50;
				attackScores[0] += 50;
				attackScores[2] += 50;
				attackScores[7] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[35] += 50;
					attackScores[8] += 50;
					attackScores[5] += 50;
					attackScores[33] += 50;
					attackScores[10] += 50;
					attackScores[17] += 50;
					attackScores[23] += 50;
					attackScores[19] += 50;
					attackScores[12] += 50;
					attackScores[4] += 50;
					attackScores[0] += 50;
					attackScores[2] += 50;
					attackScores[7] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[35] += 50;
					attackScores[8] += 50;
					attackScores[5] += 50;
					attackScores[33] += 50;
					attackScores[10] += 50;
					attackScores[17] += 50;
					attackScores[23] += 50;
					attackScores[19] += 50;
					attackScores[12] += 50;
					attackScores[4] += 50;
					attackScores[0] += 50;
					attackScores[2] += 50;
					attackScores[7] += 50;
				}
			}
		}
		loc = G.me.translate(1, 1);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[36] += 25;
				attackScores[34] += 25;
				attackScores[6] += 25;
				attackScores[7] += 25;
				attackScores[0] += 25;
				attackScores[4] += 25;
				attackScores[12] += 25;
				attackScores[20] += 25;
				attackScores[24] += 25;
				attackScores[18] += 25;
				attackScores[11] += 25;
				attackScores[3] += 25;
				attackScores[8] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[36] += 25;
						attackScores[34] += 25;
						attackScores[6] += 25;
						attackScores[7] += 25;
						attackScores[0] += 25;
						attackScores[4] += 25;
						attackScores[12] += 25;
						attackScores[20] += 25;
						attackScores[24] += 25;
						attackScores[18] += 25;
						attackScores[11] += 25;
						attackScores[3] += 25;
						attackScores[8] += 25;
					} else {
						attackScores[36] += G.paintPerChips() * 500;
						attackScores[34] += G.paintPerChips() * 500;
						attackScores[6] += G.paintPerChips() * 500;
						attackScores[7] += G.paintPerChips() * 500;
						attackScores[0] += G.paintPerChips() * 500;
						attackScores[4] += G.paintPerChips() * 500;
						attackScores[12] += G.paintPerChips() * 500;
						attackScores[20] += G.paintPerChips() * 500;
						attackScores[24] += G.paintPerChips() * 500;
						attackScores[18] += G.paintPerChips() * 500;
						attackScores[11] += G.paintPerChips() * 500;
						attackScores[3] += G.paintPerChips() * 500;
						attackScores[8] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[36] += 25;
					attackScores[34] += 25;
					attackScores[6] += 25;
					attackScores[7] += 25;
					attackScores[0] += 25;
					attackScores[4] += 25;
					attackScores[12] += 25;
					attackScores[20] += 25;
					attackScores[24] += 25;
					attackScores[18] += 25;
					attackScores[11] += 25;
					attackScores[3] += 25;
					attackScores[8] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[0] += 50;
				attackScores[4] += 50;
				attackScores[12] += 50;
				attackScores[20] += 50;
				attackScores[24] += 50;
				attackScores[18] += 50;
				attackScores[11] += 50;
				attackScores[3] += 50;
				attackScores[8] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[36] += 50;
					attackScores[34] += 50;
					attackScores[6] += 50;
					attackScores[7] += 50;
					attackScores[0] += 50;
					attackScores[4] += 50;
					attackScores[12] += 50;
					attackScores[20] += 50;
					attackScores[24] += 50;
					attackScores[18] += 50;
					attackScores[11] += 50;
					attackScores[3] += 50;
					attackScores[8] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[36] += 50;
					attackScores[34] += 50;
					attackScores[6] += 50;
					attackScores[7] += 50;
					attackScores[0] += 50;
					attackScores[4] += 50;
					attackScores[12] += 50;
					attackScores[20] += 50;
					attackScores[24] += 50;
					attackScores[18] += 50;
					attackScores[11] += 50;
					attackScores[3] += 50;
					attackScores[8] += 50;
				}
			}
		}
		loc = G.me.translate(-2, 0);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[0] += 25;
				attackScores[22] += 25;
				attackScores[21] += 25;
				attackScores[29] += 25;
				attackScores[13] += 25;
				attackScores[5] += 25;
				attackScores[1] += 25;
				attackScores[6] += 25;
				attackScores[14] += 25;
				attackScores[30] += 25;
				attackScores[25] += 25;
				attackScores[9] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[0] += 25;
						attackScores[22] += 25;
						attackScores[21] += 25;
						attackScores[29] += 25;
						attackScores[13] += 25;
						attackScores[5] += 25;
						attackScores[1] += 25;
						attackScores[6] += 25;
						attackScores[14] += 25;
						attackScores[30] += 25;
						attackScores[25] += 25;
						attackScores[9] += 25;
					} else {
						attackScores[0] += G.paintPerChips() * 500;
						attackScores[22] += G.paintPerChips() * 500;
						attackScores[21] += G.paintPerChips() * 500;
						attackScores[29] += G.paintPerChips() * 500;
						attackScores[13] += G.paintPerChips() * 500;
						attackScores[5] += G.paintPerChips() * 500;
						attackScores[1] += G.paintPerChips() * 500;
						attackScores[6] += G.paintPerChips() * 500;
						attackScores[14] += G.paintPerChips() * 500;
						attackScores[30] += G.paintPerChips() * 500;
						attackScores[25] += G.paintPerChips() * 500;
						attackScores[9] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[0] += 25;
					attackScores[22] += 25;
					attackScores[21] += 25;
					attackScores[29] += 25;
					attackScores[13] += 25;
					attackScores[5] += 25;
					attackScores[1] += 25;
					attackScores[6] += 25;
					attackScores[14] += 25;
					attackScores[30] += 25;
					attackScores[25] += 25;
					attackScores[9] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[29] += 50;
				attackScores[13] += 50;
				attackScores[5] += 50;
				attackScores[1] += 50;
				attackScores[6] += 50;
				attackScores[14] += 50;
				attackScores[30] += 50;
				attackScores[25] += 50;
				attackScores[9] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[0] += 50;
					attackScores[22] += 50;
					attackScores[21] += 50;
					attackScores[29] += 50;
					attackScores[13] += 50;
					attackScores[5] += 50;
					attackScores[1] += 50;
					attackScores[6] += 50;
					attackScores[14] += 50;
					attackScores[30] += 50;
					attackScores[25] += 50;
					attackScores[9] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[0] += 50;
					attackScores[22] += 50;
					attackScores[21] += 50;
					attackScores[29] += 50;
					attackScores[13] += 50;
					attackScores[5] += 50;
					attackScores[1] += 50;
					attackScores[6] += 50;
					attackScores[14] += 50;
					attackScores[30] += 50;
					attackScores[25] += 50;
					attackScores[9] += 50;
				}
			}
		}
		loc = G.me.translate(0, -2);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[23] += 25;
				attackScores[0] += 25;
				attackScores[21] += 25;
				attackScores[31] += 25;
				attackScores[26] += 25;
				attackScores[33] += 25;
				attackScores[17] += 25;
				attackScores[7] += 25;
				attackScores[2] += 25;
				attackScores[5] += 25;
				attackScores[15] += 25;
				attackScores[10] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[23] += 25;
						attackScores[0] += 25;
						attackScores[21] += 25;
						attackScores[31] += 25;
						attackScores[26] += 25;
						attackScores[33] += 25;
						attackScores[17] += 25;
						attackScores[7] += 25;
						attackScores[2] += 25;
						attackScores[5] += 25;
						attackScores[15] += 25;
						attackScores[10] += 25;
					} else {
						attackScores[23] += G.paintPerChips() * 500;
						attackScores[0] += G.paintPerChips() * 500;
						attackScores[21] += G.paintPerChips() * 500;
						attackScores[31] += G.paintPerChips() * 500;
						attackScores[26] += G.paintPerChips() * 500;
						attackScores[33] += G.paintPerChips() * 500;
						attackScores[17] += G.paintPerChips() * 500;
						attackScores[7] += G.paintPerChips() * 500;
						attackScores[2] += G.paintPerChips() * 500;
						attackScores[5] += G.paintPerChips() * 500;
						attackScores[15] += G.paintPerChips() * 500;
						attackScores[10] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[23] += 25;
					attackScores[0] += 25;
					attackScores[21] += 25;
					attackScores[31] += 25;
					attackScores[26] += 25;
					attackScores[33] += 25;
					attackScores[17] += 25;
					attackScores[7] += 25;
					attackScores[2] += 25;
					attackScores[5] += 25;
					attackScores[15] += 25;
					attackScores[10] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[31] += 50;
				attackScores[26] += 50;
				attackScores[33] += 50;
				attackScores[17] += 50;
				attackScores[7] += 50;
				attackScores[2] += 50;
				attackScores[5] += 50;
				attackScores[15] += 50;
				attackScores[10] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[23] += 50;
					attackScores[0] += 50;
					attackScores[21] += 50;
					attackScores[31] += 50;
					attackScores[26] += 50;
					attackScores[33] += 50;
					attackScores[17] += 50;
					attackScores[7] += 50;
					attackScores[2] += 50;
					attackScores[5] += 50;
					attackScores[15] += 50;
					attackScores[10] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[23] += 50;
					attackScores[0] += 50;
					attackScores[21] += 50;
					attackScores[31] += 50;
					attackScores[26] += 50;
					attackScores[33] += 50;
					attackScores[17] += 50;
					attackScores[7] += 50;
					attackScores[2] += 50;
					attackScores[5] += 50;
					attackScores[15] += 50;
					attackScores[10] += 50;
				}
			}
		}
		loc = G.me.translate(0, 2);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[24] += 25;
				attackScores[22] += 25;
				attackScores[0] += 25;
				attackScores[6] += 25;
				attackScores[3] += 25;
				attackScores[8] += 25;
				attackScores[18] += 25;
				attackScores[34] += 25;
				attackScores[27] += 25;
				attackScores[32] += 25;
				attackScores[16] += 25;
				attackScores[11] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[24] += 25;
						attackScores[22] += 25;
						attackScores[0] += 25;
						attackScores[6] += 25;
						attackScores[3] += 25;
						attackScores[8] += 25;
						attackScores[18] += 25;
						attackScores[34] += 25;
						attackScores[27] += 25;
						attackScores[32] += 25;
						attackScores[16] += 25;
						attackScores[11] += 25;
					} else {
						attackScores[24] += G.paintPerChips() * 500;
						attackScores[22] += G.paintPerChips() * 500;
						attackScores[0] += G.paintPerChips() * 500;
						attackScores[6] += G.paintPerChips() * 500;
						attackScores[3] += G.paintPerChips() * 500;
						attackScores[8] += G.paintPerChips() * 500;
						attackScores[18] += G.paintPerChips() * 500;
						attackScores[34] += G.paintPerChips() * 500;
						attackScores[27] += G.paintPerChips() * 500;
						attackScores[32] += G.paintPerChips() * 500;
						attackScores[16] += G.paintPerChips() * 500;
						attackScores[11] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[24] += 25;
					attackScores[22] += 25;
					attackScores[0] += 25;
					attackScores[6] += 25;
					attackScores[3] += 25;
					attackScores[8] += 25;
					attackScores[18] += 25;
					attackScores[34] += 25;
					attackScores[27] += 25;
					attackScores[32] += 25;
					attackScores[16] += 25;
					attackScores[11] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[6] += 50;
				attackScores[3] += 50;
				attackScores[8] += 50;
				attackScores[18] += 50;
				attackScores[34] += 50;
				attackScores[27] += 50;
				attackScores[32] += 50;
				attackScores[16] += 50;
				attackScores[11] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[24] += 50;
					attackScores[22] += 50;
					attackScores[0] += 50;
					attackScores[6] += 50;
					attackScores[3] += 50;
					attackScores[8] += 50;
					attackScores[18] += 50;
					attackScores[34] += 50;
					attackScores[27] += 50;
					attackScores[32] += 50;
					attackScores[16] += 50;
					attackScores[11] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[24] += 50;
					attackScores[22] += 50;
					attackScores[0] += 50;
					attackScores[6] += 50;
					attackScores[3] += 50;
					attackScores[8] += 50;
					attackScores[18] += 50;
					attackScores[34] += 50;
					attackScores[27] += 50;
					attackScores[32] += 50;
					attackScores[16] += 50;
					attackScores[11] += 50;
				}
			}
		}
		loc = G.me.translate(2, 0);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[24] += 25;
				attackScores[0] += 25;
				attackScores[23] += 25;
				attackScores[7] += 25;
				attackScores[19] += 25;
				attackScores[35] += 25;
				attackScores[28] += 25;
				attackScores[36] += 25;
				attackScores[20] += 25;
				attackScores[8] += 25;
				attackScores[4] += 25;
				attackScores[12] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[24] += 25;
						attackScores[0] += 25;
						attackScores[23] += 25;
						attackScores[7] += 25;
						attackScores[19] += 25;
						attackScores[35] += 25;
						attackScores[28] += 25;
						attackScores[36] += 25;
						attackScores[20] += 25;
						attackScores[8] += 25;
						attackScores[4] += 25;
						attackScores[12] += 25;
					} else {
						attackScores[24] += G.paintPerChips() * 500;
						attackScores[0] += G.paintPerChips() * 500;
						attackScores[23] += G.paintPerChips() * 500;
						attackScores[7] += G.paintPerChips() * 500;
						attackScores[19] += G.paintPerChips() * 500;
						attackScores[35] += G.paintPerChips() * 500;
						attackScores[28] += G.paintPerChips() * 500;
						attackScores[36] += G.paintPerChips() * 500;
						attackScores[20] += G.paintPerChips() * 500;
						attackScores[8] += G.paintPerChips() * 500;
						attackScores[4] += G.paintPerChips() * 500;
						attackScores[12] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[24] += 25;
					attackScores[0] += 25;
					attackScores[23] += 25;
					attackScores[7] += 25;
					attackScores[19] += 25;
					attackScores[35] += 25;
					attackScores[28] += 25;
					attackScores[36] += 25;
					attackScores[20] += 25;
					attackScores[8] += 25;
					attackScores[4] += 25;
					attackScores[12] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[7] += 50;
				attackScores[19] += 50;
				attackScores[35] += 50;
				attackScores[28] += 50;
				attackScores[36] += 50;
				attackScores[20] += 50;
				attackScores[8] += 50;
				attackScores[4] += 50;
				attackScores[12] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[24] += 50;
					attackScores[0] += 50;
					attackScores[23] += 50;
					attackScores[7] += 50;
					attackScores[19] += 50;
					attackScores[35] += 50;
					attackScores[28] += 50;
					attackScores[36] += 50;
					attackScores[20] += 50;
					attackScores[8] += 50;
					attackScores[4] += 50;
					attackScores[12] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[24] += 50;
					attackScores[0] += 50;
					attackScores[23] += 50;
					attackScores[7] += 50;
					attackScores[19] += 50;
					attackScores[35] += 50;
					attackScores[28] += 50;
					attackScores[36] += 50;
					attackScores[20] += 50;
					attackScores[8] += 50;
					attackScores[4] += 50;
					attackScores[12] += 50;
				}
			}
		}
		loc = G.me.translate(-2, -1);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[2] += 25;
				attackScores[14] += 25;
				attackScores[21] += 25;
				attackScores[15] += 25;
				attackScores[5] += 25;
				attackScores[1] += 25;
				attackScores[9] += 25;
				attackScores[25] += 25;
				attackScores[29] += 25;
				attackScores[13] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[2] += 25;
						attackScores[14] += 25;
						attackScores[21] += 25;
						attackScores[15] += 25;
						attackScores[5] += 25;
						attackScores[1] += 25;
						attackScores[9] += 25;
						attackScores[25] += 25;
						attackScores[29] += 25;
						attackScores[13] += 25;
					} else {
						attackScores[2] += G.paintPerChips() * 500;
						attackScores[14] += G.paintPerChips() * 500;
						attackScores[21] += G.paintPerChips() * 500;
						attackScores[15] += G.paintPerChips() * 500;
						attackScores[5] += G.paintPerChips() * 500;
						attackScores[1] += G.paintPerChips() * 500;
						attackScores[9] += G.paintPerChips() * 500;
						attackScores[25] += G.paintPerChips() * 500;
						attackScores[29] += G.paintPerChips() * 500;
						attackScores[13] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[2] += 25;
					attackScores[14] += 25;
					attackScores[21] += 25;
					attackScores[15] += 25;
					attackScores[5] += 25;
					attackScores[1] += 25;
					attackScores[9] += 25;
					attackScores[25] += 25;
					attackScores[29] += 25;
					attackScores[13] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[21] += 50;
				attackScores[15] += 50;
				attackScores[5] += 50;
				attackScores[1] += 50;
				attackScores[9] += 50;
				attackScores[25] += 50;
				attackScores[29] += 50;
				attackScores[13] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[2] += 50;
					attackScores[14] += 50;
					attackScores[21] += 50;
					attackScores[15] += 50;
					attackScores[5] += 50;
					attackScores[1] += 50;
					attackScores[9] += 50;
					attackScores[25] += 50;
					attackScores[29] += 50;
					attackScores[13] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[2] += 50;
					attackScores[14] += 50;
					attackScores[21] += 50;
					attackScores[15] += 50;
					attackScores[5] += 50;
					attackScores[1] += 50;
					attackScores[9] += 50;
					attackScores[25] += 50;
					attackScores[29] += 50;
					attackScores[13] += 50;
				}
			}
		}
		loc = G.me.translate(-2, 1);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[3] += 25;
				attackScores[13] += 25;
				attackScores[25] += 25;
				attackScores[9] += 25;
				attackScores[1] += 25;
				attackScores[6] += 25;
				attackScores[16] += 25;
				attackScores[22] += 25;
				attackScores[30] += 25;
				attackScores[14] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[3] += 25;
						attackScores[13] += 25;
						attackScores[25] += 25;
						attackScores[9] += 25;
						attackScores[1] += 25;
						attackScores[6] += 25;
						attackScores[16] += 25;
						attackScores[22] += 25;
						attackScores[30] += 25;
						attackScores[14] += 25;
					} else {
						attackScores[3] += G.paintPerChips() * 500;
						attackScores[13] += G.paintPerChips() * 500;
						attackScores[25] += G.paintPerChips() * 500;
						attackScores[9] += G.paintPerChips() * 500;
						attackScores[1] += G.paintPerChips() * 500;
						attackScores[6] += G.paintPerChips() * 500;
						attackScores[16] += G.paintPerChips() * 500;
						attackScores[22] += G.paintPerChips() * 500;
						attackScores[30] += G.paintPerChips() * 500;
						attackScores[14] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[3] += 25;
					attackScores[13] += 25;
					attackScores[25] += 25;
					attackScores[9] += 25;
					attackScores[1] += 25;
					attackScores[6] += 25;
					attackScores[16] += 25;
					attackScores[22] += 25;
					attackScores[30] += 25;
					attackScores[14] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[25] += 50;
				attackScores[9] += 50;
				attackScores[1] += 50;
				attackScores[6] += 50;
				attackScores[16] += 50;
				attackScores[22] += 50;
				attackScores[30] += 50;
				attackScores[14] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[3] += 50;
					attackScores[13] += 50;
					attackScores[25] += 50;
					attackScores[9] += 50;
					attackScores[1] += 50;
					attackScores[6] += 50;
					attackScores[16] += 50;
					attackScores[22] += 50;
					attackScores[30] += 50;
					attackScores[14] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[3] += 50;
					attackScores[13] += 50;
					attackScores[25] += 50;
					attackScores[9] += 50;
					attackScores[1] += 50;
					attackScores[6] += 50;
					attackScores[16] += 50;
					attackScores[22] += 50;
					attackScores[30] += 50;
					attackScores[14] += 50;
				}
			}
		}
		loc = G.me.translate(-1, -2);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[17] += 25;
				attackScores[1] += 25;
				attackScores[31] += 25;
				attackScores[26] += 25;
				attackScores[10] += 25;
				attackScores[2] += 25;
				attackScores[5] += 25;
				attackScores[13] += 25;
				attackScores[21] += 25;
				attackScores[15] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[17] += 25;
						attackScores[1] += 25;
						attackScores[31] += 25;
						attackScores[26] += 25;
						attackScores[10] += 25;
						attackScores[2] += 25;
						attackScores[5] += 25;
						attackScores[13] += 25;
						attackScores[21] += 25;
						attackScores[15] += 25;
					} else {
						attackScores[17] += G.paintPerChips() * 500;
						attackScores[1] += G.paintPerChips() * 500;
						attackScores[31] += G.paintPerChips() * 500;
						attackScores[26] += G.paintPerChips() * 500;
						attackScores[10] += G.paintPerChips() * 500;
						attackScores[2] += G.paintPerChips() * 500;
						attackScores[5] += G.paintPerChips() * 500;
						attackScores[13] += G.paintPerChips() * 500;
						attackScores[21] += G.paintPerChips() * 500;
						attackScores[15] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[17] += 25;
					attackScores[1] += 25;
					attackScores[31] += 25;
					attackScores[26] += 25;
					attackScores[10] += 25;
					attackScores[2] += 25;
					attackScores[5] += 25;
					attackScores[13] += 25;
					attackScores[21] += 25;
					attackScores[15] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[31] += 50;
				attackScores[26] += 50;
				attackScores[10] += 50;
				attackScores[2] += 50;
				attackScores[5] += 50;
				attackScores[13] += 50;
				attackScores[21] += 50;
				attackScores[15] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[17] += 50;
					attackScores[1] += 50;
					attackScores[31] += 50;
					attackScores[26] += 50;
					attackScores[10] += 50;
					attackScores[2] += 50;
					attackScores[5] += 50;
					attackScores[13] += 50;
					attackScores[21] += 50;
					attackScores[15] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[17] += 50;
					attackScores[1] += 50;
					attackScores[31] += 50;
					attackScores[26] += 50;
					attackScores[10] += 50;
					attackScores[2] += 50;
					attackScores[5] += 50;
					attackScores[13] += 50;
					attackScores[21] += 50;
					attackScores[15] += 50;
				}
			}
		}
		loc = G.me.translate(-1, 2);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[18] += 25;
				attackScores[1] += 25;
				attackScores[14] += 25;
				attackScores[6] += 25;
				attackScores[3] += 25;
				attackScores[11] += 25;
				attackScores[27] += 25;
				attackScores[32] += 25;
				attackScores[22] += 25;
				attackScores[16] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[18] += 25;
						attackScores[1] += 25;
						attackScores[14] += 25;
						attackScores[6] += 25;
						attackScores[3] += 25;
						attackScores[11] += 25;
						attackScores[27] += 25;
						attackScores[32] += 25;
						attackScores[22] += 25;
						attackScores[16] += 25;
					} else {
						attackScores[18] += G.paintPerChips() * 500;
						attackScores[1] += G.paintPerChips() * 500;
						attackScores[14] += G.paintPerChips() * 500;
						attackScores[6] += G.paintPerChips() * 500;
						attackScores[3] += G.paintPerChips() * 500;
						attackScores[11] += G.paintPerChips() * 500;
						attackScores[27] += G.paintPerChips() * 500;
						attackScores[32] += G.paintPerChips() * 500;
						attackScores[22] += G.paintPerChips() * 500;
						attackScores[16] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[18] += 25;
					attackScores[1] += 25;
					attackScores[14] += 25;
					attackScores[6] += 25;
					attackScores[3] += 25;
					attackScores[11] += 25;
					attackScores[27] += 25;
					attackScores[32] += 25;
					attackScores[22] += 25;
					attackScores[16] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[14] += 50;
				attackScores[6] += 50;
				attackScores[3] += 50;
				attackScores[11] += 50;
				attackScores[27] += 50;
				attackScores[32] += 50;
				attackScores[22] += 50;
				attackScores[16] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[18] += 50;
					attackScores[1] += 50;
					attackScores[14] += 50;
					attackScores[6] += 50;
					attackScores[3] += 50;
					attackScores[11] += 50;
					attackScores[27] += 50;
					attackScores[32] += 50;
					attackScores[22] += 50;
					attackScores[16] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[18] += 50;
					attackScores[1] += 50;
					attackScores[14] += 50;
					attackScores[6] += 50;
					attackScores[3] += 50;
					attackScores[11] += 50;
					attackScores[27] += 50;
					attackScores[32] += 50;
					attackScores[22] += 50;
					attackScores[16] += 50;
				}
			}
		}
		loc = G.me.translate(1, -2);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[4] += 25;
				attackScores[15] += 25;
				attackScores[26] += 25;
				attackScores[33] += 25;
				attackScores[23] += 25;
				attackScores[19] += 25;
				attackScores[7] += 25;
				attackScores[2] += 25;
				attackScores[10] += 25;
				attackScores[17] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[4] += 25;
						attackScores[15] += 25;
						attackScores[26] += 25;
						attackScores[33] += 25;
						attackScores[23] += 25;
						attackScores[19] += 25;
						attackScores[7] += 25;
						attackScores[2] += 25;
						attackScores[10] += 25;
						attackScores[17] += 25;
					} else {
						attackScores[4] += G.paintPerChips() * 500;
						attackScores[15] += G.paintPerChips() * 500;
						attackScores[26] += G.paintPerChips() * 500;
						attackScores[33] += G.paintPerChips() * 500;
						attackScores[23] += G.paintPerChips() * 500;
						attackScores[19] += G.paintPerChips() * 500;
						attackScores[7] += G.paintPerChips() * 500;
						attackScores[2] += G.paintPerChips() * 500;
						attackScores[10] += G.paintPerChips() * 500;
						attackScores[17] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[4] += 25;
					attackScores[15] += 25;
					attackScores[26] += 25;
					attackScores[33] += 25;
					attackScores[23] += 25;
					attackScores[19] += 25;
					attackScores[7] += 25;
					attackScores[2] += 25;
					attackScores[10] += 25;
					attackScores[17] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[26] += 50;
				attackScores[33] += 50;
				attackScores[23] += 50;
				attackScores[19] += 50;
				attackScores[7] += 50;
				attackScores[2] += 50;
				attackScores[10] += 50;
				attackScores[17] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[4] += 50;
					attackScores[15] += 50;
					attackScores[26] += 50;
					attackScores[33] += 50;
					attackScores[23] += 50;
					attackScores[19] += 50;
					attackScores[7] += 50;
					attackScores[2] += 50;
					attackScores[10] += 50;
					attackScores[17] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[4] += 50;
					attackScores[15] += 50;
					attackScores[26] += 50;
					attackScores[33] += 50;
					attackScores[23] += 50;
					attackScores[19] += 50;
					attackScores[7] += 50;
					attackScores[2] += 50;
					attackScores[10] += 50;
					attackScores[17] += 50;
				}
			}
		}
		loc = G.me.translate(1, 2);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[16] += 25;
				attackScores[4] += 25;
				attackScores[3] += 25;
				attackScores[8] += 25;
				attackScores[20] += 25;
				attackScores[24] += 25;
				attackScores[34] += 25;
				attackScores[27] += 25;
				attackScores[11] += 25;
				attackScores[18] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[16] += 25;
						attackScores[4] += 25;
						attackScores[3] += 25;
						attackScores[8] += 25;
						attackScores[20] += 25;
						attackScores[24] += 25;
						attackScores[34] += 25;
						attackScores[27] += 25;
						attackScores[11] += 25;
						attackScores[18] += 25;
					} else {
						attackScores[16] += G.paintPerChips() * 500;
						attackScores[4] += G.paintPerChips() * 500;
						attackScores[3] += G.paintPerChips() * 500;
						attackScores[8] += G.paintPerChips() * 500;
						attackScores[20] += G.paintPerChips() * 500;
						attackScores[24] += G.paintPerChips() * 500;
						attackScores[34] += G.paintPerChips() * 500;
						attackScores[27] += G.paintPerChips() * 500;
						attackScores[11] += G.paintPerChips() * 500;
						attackScores[18] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[16] += 25;
					attackScores[4] += 25;
					attackScores[3] += 25;
					attackScores[8] += 25;
					attackScores[20] += 25;
					attackScores[24] += 25;
					attackScores[34] += 25;
					attackScores[27] += 25;
					attackScores[11] += 25;
					attackScores[18] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[3] += 50;
				attackScores[8] += 50;
				attackScores[20] += 50;
				attackScores[24] += 50;
				attackScores[34] += 50;
				attackScores[27] += 50;
				attackScores[11] += 50;
				attackScores[18] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[16] += 50;
					attackScores[4] += 50;
					attackScores[3] += 50;
					attackScores[8] += 50;
					attackScores[20] += 50;
					attackScores[24] += 50;
					attackScores[34] += 50;
					attackScores[27] += 50;
					attackScores[11] += 50;
					attackScores[18] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[16] += 50;
					attackScores[4] += 50;
					attackScores[3] += 50;
					attackScores[8] += 50;
					attackScores[20] += 50;
					attackScores[24] += 50;
					attackScores[34] += 50;
					attackScores[27] += 50;
					attackScores[11] += 50;
					attackScores[18] += 50;
				}
			}
		}
		loc = G.me.translate(2, -1);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[20] += 25;
				attackScores[2] += 25;
				attackScores[17] += 25;
				attackScores[23] += 25;
				attackScores[35] += 25;
				attackScores[28] += 25;
				attackScores[12] += 25;
				attackScores[4] += 25;
				attackScores[7] += 25;
				attackScores[19] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[20] += 25;
						attackScores[2] += 25;
						attackScores[17] += 25;
						attackScores[23] += 25;
						attackScores[35] += 25;
						attackScores[28] += 25;
						attackScores[12] += 25;
						attackScores[4] += 25;
						attackScores[7] += 25;
						attackScores[19] += 25;
					} else {
						attackScores[20] += G.paintPerChips() * 500;
						attackScores[2] += G.paintPerChips() * 500;
						attackScores[17] += G.paintPerChips() * 500;
						attackScores[23] += G.paintPerChips() * 500;
						attackScores[35] += G.paintPerChips() * 500;
						attackScores[28] += G.paintPerChips() * 500;
						attackScores[12] += G.paintPerChips() * 500;
						attackScores[4] += G.paintPerChips() * 500;
						attackScores[7] += G.paintPerChips() * 500;
						attackScores[19] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[20] += 25;
					attackScores[2] += 25;
					attackScores[17] += 25;
					attackScores[23] += 25;
					attackScores[35] += 25;
					attackScores[28] += 25;
					attackScores[12] += 25;
					attackScores[4] += 25;
					attackScores[7] += 25;
					attackScores[19] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[17] += 50;
				attackScores[23] += 50;
				attackScores[35] += 50;
				attackScores[28] += 50;
				attackScores[12] += 50;
				attackScores[4] += 50;
				attackScores[7] += 50;
				attackScores[19] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[20] += 50;
					attackScores[2] += 50;
					attackScores[17] += 50;
					attackScores[23] += 50;
					attackScores[35] += 50;
					attackScores[28] += 50;
					attackScores[12] += 50;
					attackScores[4] += 50;
					attackScores[7] += 50;
					attackScores[19] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[20] += 50;
					attackScores[2] += 50;
					attackScores[17] += 50;
					attackScores[23] += 50;
					attackScores[35] += 50;
					attackScores[28] += 50;
					attackScores[12] += 50;
					attackScores[4] += 50;
					attackScores[7] += 50;
					attackScores[19] += 50;
				}
			}
		}
		loc = G.me.translate(2, 1);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[3] += 25;
				attackScores[19] += 25;
				attackScores[4] += 25;
				attackScores[12] += 25;
				attackScores[28] += 25;
				attackScores[36] += 25;
				attackScores[24] += 25;
				attackScores[18] += 25;
				attackScores[8] += 25;
				attackScores[20] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[3] += 25;
						attackScores[19] += 25;
						attackScores[4] += 25;
						attackScores[12] += 25;
						attackScores[28] += 25;
						attackScores[36] += 25;
						attackScores[24] += 25;
						attackScores[18] += 25;
						attackScores[8] += 25;
						attackScores[20] += 25;
					} else {
						attackScores[3] += G.paintPerChips() * 500;
						attackScores[19] += G.paintPerChips() * 500;
						attackScores[4] += G.paintPerChips() * 500;
						attackScores[12] += G.paintPerChips() * 500;
						attackScores[28] += G.paintPerChips() * 500;
						attackScores[36] += G.paintPerChips() * 500;
						attackScores[24] += G.paintPerChips() * 500;
						attackScores[18] += G.paintPerChips() * 500;
						attackScores[8] += G.paintPerChips() * 500;
						attackScores[20] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[3] += 25;
					attackScores[19] += 25;
					attackScores[4] += 25;
					attackScores[12] += 25;
					attackScores[28] += 25;
					attackScores[36] += 25;
					attackScores[24] += 25;
					attackScores[18] += 25;
					attackScores[8] += 25;
					attackScores[20] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[4] += 50;
				attackScores[12] += 50;
				attackScores[28] += 50;
				attackScores[36] += 50;
				attackScores[24] += 50;
				attackScores[18] += 50;
				attackScores[8] += 50;
				attackScores[20] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[3] += 50;
					attackScores[19] += 50;
					attackScores[4] += 50;
					attackScores[12] += 50;
					attackScores[28] += 50;
					attackScores[36] += 50;
					attackScores[24] += 50;
					attackScores[18] += 50;
					attackScores[8] += 50;
					attackScores[20] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[3] += 50;
					attackScores[19] += 50;
					attackScores[4] += 50;
					attackScores[12] += 50;
					attackScores[28] += 50;
					attackScores[36] += 50;
					attackScores[24] += 50;
					attackScores[18] += 50;
					attackScores[8] += 50;
					attackScores[20] += 50;
				}
			}
		}
		loc = G.me.translate(-2, -2);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[10] += 25;
				attackScores[9] += 25;
				attackScores[31] += 25;
				attackScores[15] += 25;
				attackScores[5] += 25;
				attackScores[13] += 25;
				attackScores[29] += 25;
				attackScores[21] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[10] += 25;
						attackScores[9] += 25;
						attackScores[31] += 25;
						attackScores[15] += 25;
						attackScores[5] += 25;
						attackScores[13] += 25;
						attackScores[29] += 25;
						attackScores[21] += 25;
					} else {
						attackScores[10] += G.paintPerChips() * 500;
						attackScores[9] += G.paintPerChips() * 500;
						attackScores[31] += G.paintPerChips() * 500;
						attackScores[15] += G.paintPerChips() * 500;
						attackScores[5] += G.paintPerChips() * 500;
						attackScores[13] += G.paintPerChips() * 500;
						attackScores[29] += G.paintPerChips() * 500;
						attackScores[21] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[10] += 25;
					attackScores[9] += 25;
					attackScores[31] += 25;
					attackScores[15] += 25;
					attackScores[5] += 25;
					attackScores[13] += 25;
					attackScores[29] += 25;
					attackScores[21] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[31] += 50;
				attackScores[15] += 50;
				attackScores[5] += 50;
				attackScores[13] += 50;
				attackScores[29] += 50;
				attackScores[21] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[10] += 50;
					attackScores[9] += 50;
					attackScores[31] += 50;
					attackScores[15] += 50;
					attackScores[5] += 50;
					attackScores[13] += 50;
					attackScores[29] += 50;
					attackScores[21] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[10] += 50;
					attackScores[9] += 50;
					attackScores[31] += 50;
					attackScores[15] += 50;
					attackScores[5] += 50;
					attackScores[13] += 50;
					attackScores[29] += 50;
					attackScores[21] += 50;
				}
			}
		}
		loc = G.me.translate(-2, 2);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[11] += 25;
				attackScores[9] += 25;
				attackScores[30] += 25;
				attackScores[14] += 25;
				attackScores[6] += 25;
				attackScores[16] += 25;
				attackScores[32] += 25;
				attackScores[22] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[11] += 25;
						attackScores[9] += 25;
						attackScores[30] += 25;
						attackScores[14] += 25;
						attackScores[6] += 25;
						attackScores[16] += 25;
						attackScores[32] += 25;
						attackScores[22] += 25;
					} else {
						attackScores[11] += G.paintPerChips() * 500;
						attackScores[9] += G.paintPerChips() * 500;
						attackScores[30] += G.paintPerChips() * 500;
						attackScores[14] += G.paintPerChips() * 500;
						attackScores[6] += G.paintPerChips() * 500;
						attackScores[16] += G.paintPerChips() * 500;
						attackScores[32] += G.paintPerChips() * 500;
						attackScores[22] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[11] += 25;
					attackScores[9] += 25;
					attackScores[30] += 25;
					attackScores[14] += 25;
					attackScores[6] += 25;
					attackScores[16] += 25;
					attackScores[32] += 25;
					attackScores[22] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[30] += 50;
				attackScores[14] += 50;
				attackScores[6] += 50;
				attackScores[16] += 50;
				attackScores[32] += 50;
				attackScores[22] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[11] += 50;
					attackScores[9] += 50;
					attackScores[30] += 50;
					attackScores[14] += 50;
					attackScores[6] += 50;
					attackScores[16] += 50;
					attackScores[32] += 50;
					attackScores[22] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[11] += 50;
					attackScores[9] += 50;
					attackScores[30] += 50;
					attackScores[14] += 50;
					attackScores[6] += 50;
					attackScores[16] += 50;
					attackScores[32] += 50;
					attackScores[22] += 50;
				}
			}
		}
    }

    public static void exploreAttackScores2() throws Exception {
        MapLocation[] ruins = G.rc.senseNearbyRuins(-1);
        StringBuilder ruinsString = new StringBuilder();
        for (int i = ruins.length; --i >= 0;) {
            ruinsString.append(ruins[i].toString());
        }
        MapLocation loc = G.me.translate(2, -2);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[12] += 25;
				attackScores[10] += 25;
				attackScores[33] += 25;
				attackScores[35] += 25;
				attackScores[19] += 25;
				attackScores[7] += 25;
				attackScores[17] += 25;
				attackScores[23] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[12] += 25;
						attackScores[10] += 25;
						attackScores[33] += 25;
						attackScores[35] += 25;
						attackScores[19] += 25;
						attackScores[7] += 25;
						attackScores[17] += 25;
						attackScores[23] += 25;
					} else {
						attackScores[12] += G.paintPerChips() * 500;
						attackScores[10] += G.paintPerChips() * 500;
						attackScores[33] += G.paintPerChips() * 500;
						attackScores[35] += G.paintPerChips() * 500;
						attackScores[19] += G.paintPerChips() * 500;
						attackScores[7] += G.paintPerChips() * 500;
						attackScores[17] += G.paintPerChips() * 500;
						attackScores[23] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[12] += 25;
					attackScores[10] += 25;
					attackScores[33] += 25;
					attackScores[35] += 25;
					attackScores[19] += 25;
					attackScores[7] += 25;
					attackScores[17] += 25;
					attackScores[23] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[33] += 50;
				attackScores[35] += 50;
				attackScores[19] += 50;
				attackScores[7] += 50;
				attackScores[17] += 50;
				attackScores[23] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[12] += 50;
					attackScores[10] += 50;
					attackScores[33] += 50;
					attackScores[35] += 50;
					attackScores[19] += 50;
					attackScores[7] += 50;
					attackScores[17] += 50;
					attackScores[23] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[12] += 50;
					attackScores[10] += 50;
					attackScores[33] += 50;
					attackScores[35] += 50;
					attackScores[19] += 50;
					attackScores[7] += 50;
					attackScores[17] += 50;
					attackScores[23] += 50;
				}
			}
		}
		loc = G.me.translate(2, 2);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[11] += 25;
				attackScores[12] += 25;
				attackScores[8] += 25;
				attackScores[20] += 25;
				attackScores[36] += 25;
				attackScores[34] += 25;
				attackScores[18] += 25;
				attackScores[24] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[11] += 25;
						attackScores[12] += 25;
						attackScores[8] += 25;
						attackScores[20] += 25;
						attackScores[36] += 25;
						attackScores[34] += 25;
						attackScores[18] += 25;
						attackScores[24] += 25;
					} else {
						attackScores[11] += G.paintPerChips() * 500;
						attackScores[12] += G.paintPerChips() * 500;
						attackScores[8] += G.paintPerChips() * 500;
						attackScores[20] += G.paintPerChips() * 500;
						attackScores[36] += G.paintPerChips() * 500;
						attackScores[34] += G.paintPerChips() * 500;
						attackScores[18] += G.paintPerChips() * 500;
						attackScores[24] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[11] += 25;
					attackScores[12] += 25;
					attackScores[8] += 25;
					attackScores[20] += 25;
					attackScores[36] += 25;
					attackScores[34] += 25;
					attackScores[18] += 25;
					attackScores[24] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[8] += 50;
				attackScores[20] += 50;
				attackScores[36] += 50;
				attackScores[34] += 50;
				attackScores[18] += 50;
				attackScores[24] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[11] += 50;
					attackScores[12] += 50;
					attackScores[8] += 50;
					attackScores[20] += 50;
					attackScores[36] += 50;
					attackScores[34] += 50;
					attackScores[18] += 50;
					attackScores[24] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[11] += 50;
					attackScores[12] += 50;
					attackScores[8] += 50;
					attackScores[20] += 50;
					attackScores[36] += 50;
					attackScores[34] += 50;
					attackScores[18] += 50;
					attackScores[24] += 50;
				}
			}
		}
		loc = G.me.translate(-3, 0);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[1] += 25;
				attackScores[29] += 25;
				attackScores[13] += 25;
				attackScores[9] += 25;
				attackScores[14] += 25;
				attackScores[30] += 25;
				attackScores[25] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[1] += 25;
						attackScores[29] += 25;
						attackScores[13] += 25;
						attackScores[9] += 25;
						attackScores[14] += 25;
						attackScores[30] += 25;
						attackScores[25] += 25;
					} else {
						attackScores[1] += G.paintPerChips() * 500;
						attackScores[29] += G.paintPerChips() * 500;
						attackScores[13] += G.paintPerChips() * 500;
						attackScores[9] += G.paintPerChips() * 500;
						attackScores[14] += G.paintPerChips() * 500;
						attackScores[30] += G.paintPerChips() * 500;
						attackScores[25] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[1] += 25;
					attackScores[29] += 25;
					attackScores[13] += 25;
					attackScores[9] += 25;
					attackScores[14] += 25;
					attackScores[30] += 25;
					attackScores[25] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[29] += 50;
				attackScores[13] += 50;
				attackScores[9] += 50;
				attackScores[14] += 50;
				attackScores[30] += 50;
				attackScores[25] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[1] += 50;
					attackScores[29] += 50;
					attackScores[13] += 50;
					attackScores[9] += 50;
					attackScores[14] += 50;
					attackScores[30] += 50;
					attackScores[25] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[1] += 50;
					attackScores[29] += 50;
					attackScores[13] += 50;
					attackScores[9] += 50;
					attackScores[14] += 50;
					attackScores[30] += 50;
					attackScores[25] += 50;
				}
			}
		}
		loc = G.me.translate(0, -3);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[2] += 25;
				attackScores[33] += 25;
				attackScores[17] += 25;
				attackScores[10] += 25;
				attackScores[15] += 25;
				attackScores[31] += 25;
				attackScores[26] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[2] += 25;
						attackScores[33] += 25;
						attackScores[17] += 25;
						attackScores[10] += 25;
						attackScores[15] += 25;
						attackScores[31] += 25;
						attackScores[26] += 25;
					} else {
						attackScores[2] += G.paintPerChips() * 500;
						attackScores[33] += G.paintPerChips() * 500;
						attackScores[17] += G.paintPerChips() * 500;
						attackScores[10] += G.paintPerChips() * 500;
						attackScores[15] += G.paintPerChips() * 500;
						attackScores[31] += G.paintPerChips() * 500;
						attackScores[26] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[2] += 25;
					attackScores[33] += 25;
					attackScores[17] += 25;
					attackScores[10] += 25;
					attackScores[15] += 25;
					attackScores[31] += 25;
					attackScores[26] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[33] += 50;
				attackScores[17] += 50;
				attackScores[10] += 50;
				attackScores[15] += 50;
				attackScores[31] += 50;
				attackScores[26] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[2] += 50;
					attackScores[33] += 50;
					attackScores[17] += 50;
					attackScores[10] += 50;
					attackScores[15] += 50;
					attackScores[31] += 50;
					attackScores[26] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[2] += 50;
					attackScores[33] += 50;
					attackScores[17] += 50;
					attackScores[10] += 50;
					attackScores[15] += 50;
					attackScores[31] += 50;
					attackScores[26] += 50;
				}
			}
		}
		loc = G.me.translate(0, 3);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[3] += 25;
				attackScores[16] += 25;
				attackScores[11] += 25;
				attackScores[18] += 25;
				attackScores[34] += 25;
				attackScores[32] += 25;
				attackScores[27] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[3] += 25;
						attackScores[16] += 25;
						attackScores[11] += 25;
						attackScores[18] += 25;
						attackScores[34] += 25;
						attackScores[32] += 25;
						attackScores[27] += 25;
					} else {
						attackScores[3] += G.paintPerChips() * 500;
						attackScores[16] += G.paintPerChips() * 500;
						attackScores[11] += G.paintPerChips() * 500;
						attackScores[18] += G.paintPerChips() * 500;
						attackScores[34] += G.paintPerChips() * 500;
						attackScores[32] += G.paintPerChips() * 500;
						attackScores[27] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[3] += 25;
					attackScores[16] += 25;
					attackScores[11] += 25;
					attackScores[18] += 25;
					attackScores[34] += 25;
					attackScores[32] += 25;
					attackScores[27] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[16] += 50;
				attackScores[11] += 50;
				attackScores[18] += 50;
				attackScores[34] += 50;
				attackScores[32] += 50;
				attackScores[27] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[3] += 50;
					attackScores[16] += 50;
					attackScores[11] += 50;
					attackScores[18] += 50;
					attackScores[34] += 50;
					attackScores[32] += 50;
					attackScores[27] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[3] += 50;
					attackScores[16] += 50;
					attackScores[11] += 50;
					attackScores[18] += 50;
					attackScores[34] += 50;
					attackScores[32] += 50;
					attackScores[27] += 50;
				}
			}
		}
		loc = G.me.translate(3, 0);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[4] += 25;
				attackScores[19] += 25;
				attackScores[35] += 25;
				attackScores[36] += 25;
				attackScores[20] += 25;
				attackScores[12] += 25;
				attackScores[28] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[4] += 25;
						attackScores[19] += 25;
						attackScores[35] += 25;
						attackScores[36] += 25;
						attackScores[20] += 25;
						attackScores[12] += 25;
						attackScores[28] += 25;
					} else {
						attackScores[4] += G.paintPerChips() * 500;
						attackScores[19] += G.paintPerChips() * 500;
						attackScores[35] += G.paintPerChips() * 500;
						attackScores[36] += G.paintPerChips() * 500;
						attackScores[20] += G.paintPerChips() * 500;
						attackScores[12] += G.paintPerChips() * 500;
						attackScores[28] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[4] += 25;
					attackScores[19] += 25;
					attackScores[35] += 25;
					attackScores[36] += 25;
					attackScores[20] += 25;
					attackScores[12] += 25;
					attackScores[28] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[19] += 50;
				attackScores[35] += 50;
				attackScores[36] += 50;
				attackScores[20] += 50;
				attackScores[12] += 50;
				attackScores[28] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[4] += 50;
					attackScores[19] += 50;
					attackScores[35] += 50;
					attackScores[36] += 50;
					attackScores[20] += 50;
					attackScores[12] += 50;
					attackScores[28] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[4] += 50;
					attackScores[19] += 50;
					attackScores[35] += 50;
					attackScores[36] += 50;
					attackScores[20] += 50;
					attackScores[12] += 50;
					attackScores[28] += 50;
				}
			}
		}
		loc = G.me.translate(-3, -1);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[5] += 25;
				attackScores[30] += 25;
				attackScores[21] += 25;
				attackScores[13] += 25;
				attackScores[9] += 25;
				attackScores[25] += 25;
				attackScores[29] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[5] += 25;
						attackScores[30] += 25;
						attackScores[21] += 25;
						attackScores[13] += 25;
						attackScores[9] += 25;
						attackScores[25] += 25;
						attackScores[29] += 25;
					} else {
						attackScores[5] += G.paintPerChips() * 500;
						attackScores[30] += G.paintPerChips() * 500;
						attackScores[21] += G.paintPerChips() * 500;
						attackScores[13] += G.paintPerChips() * 500;
						attackScores[9] += G.paintPerChips() * 500;
						attackScores[25] += G.paintPerChips() * 500;
						attackScores[29] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[5] += 25;
					attackScores[30] += 25;
					attackScores[21] += 25;
					attackScores[13] += 25;
					attackScores[9] += 25;
					attackScores[25] += 25;
					attackScores[29] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[21] += 50;
				attackScores[13] += 50;
				attackScores[9] += 50;
				attackScores[25] += 50;
				attackScores[29] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[5] += 50;
					attackScores[30] += 50;
					attackScores[21] += 50;
					attackScores[13] += 50;
					attackScores[9] += 50;
					attackScores[25] += 50;
					attackScores[29] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[5] += 50;
					attackScores[30] += 50;
					attackScores[21] += 50;
					attackScores[13] += 50;
					attackScores[9] += 50;
					attackScores[25] += 50;
					attackScores[29] += 50;
				}
			}
		}
		loc = G.me.translate(-3, 1);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[6] += 25;
				attackScores[29] += 25;
				attackScores[25] += 25;
				attackScores[9] += 25;
				attackScores[14] += 25;
				attackScores[22] += 25;
				attackScores[30] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[6] += 25;
						attackScores[29] += 25;
						attackScores[25] += 25;
						attackScores[9] += 25;
						attackScores[14] += 25;
						attackScores[22] += 25;
						attackScores[30] += 25;
					} else {
						attackScores[6] += G.paintPerChips() * 500;
						attackScores[29] += G.paintPerChips() * 500;
						attackScores[25] += G.paintPerChips() * 500;
						attackScores[9] += G.paintPerChips() * 500;
						attackScores[14] += G.paintPerChips() * 500;
						attackScores[22] += G.paintPerChips() * 500;
						attackScores[30] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[6] += 25;
					attackScores[29] += 25;
					attackScores[25] += 25;
					attackScores[9] += 25;
					attackScores[14] += 25;
					attackScores[22] += 25;
					attackScores[30] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[25] += 50;
				attackScores[9] += 50;
				attackScores[14] += 50;
				attackScores[22] += 50;
				attackScores[30] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[6] += 50;
					attackScores[29] += 50;
					attackScores[25] += 50;
					attackScores[9] += 50;
					attackScores[14] += 50;
					attackScores[22] += 50;
					attackScores[30] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[6] += 50;
					attackScores[29] += 50;
					attackScores[25] += 50;
					attackScores[9] += 50;
					attackScores[14] += 50;
					attackScores[22] += 50;
					attackScores[30] += 50;
				}
			}
		}
		loc = G.me.translate(-1, -3);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[33] += 25;
				attackScores[5] += 25;
				attackScores[26] += 25;
				attackScores[10] += 25;
				attackScores[15] += 25;
				attackScores[21] += 25;
				attackScores[31] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[33] += 25;
						attackScores[5] += 25;
						attackScores[26] += 25;
						attackScores[10] += 25;
						attackScores[15] += 25;
						attackScores[21] += 25;
						attackScores[31] += 25;
					} else {
						attackScores[33] += G.paintPerChips() * 500;
						attackScores[5] += G.paintPerChips() * 500;
						attackScores[26] += G.paintPerChips() * 500;
						attackScores[10] += G.paintPerChips() * 500;
						attackScores[15] += G.paintPerChips() * 500;
						attackScores[21] += G.paintPerChips() * 500;
						attackScores[31] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[33] += 25;
					attackScores[5] += 25;
					attackScores[26] += 25;
					attackScores[10] += 25;
					attackScores[15] += 25;
					attackScores[21] += 25;
					attackScores[31] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[26] += 50;
				attackScores[10] += 50;
				attackScores[15] += 50;
				attackScores[21] += 50;
				attackScores[31] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[33] += 50;
					attackScores[5] += 50;
					attackScores[26] += 50;
					attackScores[10] += 50;
					attackScores[15] += 50;
					attackScores[21] += 50;
					attackScores[31] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[33] += 50;
					attackScores[5] += 50;
					attackScores[26] += 50;
					attackScores[10] += 50;
					attackScores[15] += 50;
					attackScores[21] += 50;
					attackScores[31] += 50;
				}
			}
		}
		loc = G.me.translate(-1, 3);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[34] += 25;
				attackScores[6] += 25;
				attackScores[22] += 25;
				attackScores[16] += 25;
				attackScores[11] += 25;
				attackScores[27] += 25;
				attackScores[32] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[34] += 25;
						attackScores[6] += 25;
						attackScores[22] += 25;
						attackScores[16] += 25;
						attackScores[11] += 25;
						attackScores[27] += 25;
						attackScores[32] += 25;
					} else {
						attackScores[34] += G.paintPerChips() * 500;
						attackScores[6] += G.paintPerChips() * 500;
						attackScores[22] += G.paintPerChips() * 500;
						attackScores[16] += G.paintPerChips() * 500;
						attackScores[11] += G.paintPerChips() * 500;
						attackScores[27] += G.paintPerChips() * 500;
						attackScores[32] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[34] += 25;
					attackScores[6] += 25;
					attackScores[22] += 25;
					attackScores[16] += 25;
					attackScores[11] += 25;
					attackScores[27] += 25;
					attackScores[32] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[22] += 50;
				attackScores[16] += 50;
				attackScores[11] += 50;
				attackScores[27] += 50;
				attackScores[32] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[34] += 50;
					attackScores[6] += 50;
					attackScores[22] += 50;
					attackScores[16] += 50;
					attackScores[11] += 50;
					attackScores[27] += 50;
					attackScores[32] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[34] += 50;
					attackScores[6] += 50;
					attackScores[22] += 50;
					attackScores[16] += 50;
					attackScores[11] += 50;
					attackScores[27] += 50;
					attackScores[32] += 50;
				}
			}
		}
		loc = G.me.translate(1, -3);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[7] += 25;
				attackScores[31] += 25;
				attackScores[23] += 25;
				attackScores[17] += 25;
				attackScores[10] += 25;
				attackScores[26] += 25;
				attackScores[33] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[7] += 25;
						attackScores[31] += 25;
						attackScores[23] += 25;
						attackScores[17] += 25;
						attackScores[10] += 25;
						attackScores[26] += 25;
						attackScores[33] += 25;
					} else {
						attackScores[7] += G.paintPerChips() * 500;
						attackScores[31] += G.paintPerChips() * 500;
						attackScores[23] += G.paintPerChips() * 500;
						attackScores[17] += G.paintPerChips() * 500;
						attackScores[10] += G.paintPerChips() * 500;
						attackScores[26] += G.paintPerChips() * 500;
						attackScores[33] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[7] += 25;
					attackScores[31] += 25;
					attackScores[23] += 25;
					attackScores[17] += 25;
					attackScores[10] += 25;
					attackScores[26] += 25;
					attackScores[33] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[23] += 50;
				attackScores[17] += 50;
				attackScores[10] += 50;
				attackScores[26] += 50;
				attackScores[33] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[7] += 50;
					attackScores[31] += 50;
					attackScores[23] += 50;
					attackScores[17] += 50;
					attackScores[10] += 50;
					attackScores[26] += 50;
					attackScores[33] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[7] += 50;
					attackScores[31] += 50;
					attackScores[23] += 50;
					attackScores[17] += 50;
					attackScores[10] += 50;
					attackScores[26] += 50;
					attackScores[33] += 50;
				}
			}
		}
		loc = G.me.translate(1, 3);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[32] += 25;
				attackScores[8] += 25;
				attackScores[11] += 25;
				attackScores[18] += 25;
				attackScores[24] += 25;
				attackScores[27] += 25;
				attackScores[34] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[32] += 25;
						attackScores[8] += 25;
						attackScores[11] += 25;
						attackScores[18] += 25;
						attackScores[24] += 25;
						attackScores[27] += 25;
						attackScores[34] += 25;
					} else {
						attackScores[32] += G.paintPerChips() * 500;
						attackScores[8] += G.paintPerChips() * 500;
						attackScores[11] += G.paintPerChips() * 500;
						attackScores[18] += G.paintPerChips() * 500;
						attackScores[24] += G.paintPerChips() * 500;
						attackScores[27] += G.paintPerChips() * 500;
						attackScores[34] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[32] += 25;
					attackScores[8] += 25;
					attackScores[11] += 25;
					attackScores[18] += 25;
					attackScores[24] += 25;
					attackScores[27] += 25;
					attackScores[34] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[11] += 50;
				attackScores[18] += 50;
				attackScores[24] += 50;
				attackScores[27] += 50;
				attackScores[34] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[32] += 50;
					attackScores[8] += 50;
					attackScores[11] += 50;
					attackScores[18] += 50;
					attackScores[24] += 50;
					attackScores[27] += 50;
					attackScores[34] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[32] += 50;
					attackScores[8] += 50;
					attackScores[11] += 50;
					attackScores[18] += 50;
					attackScores[24] += 50;
					attackScores[27] += 50;
					attackScores[34] += 50;
				}
			}
		}
		loc = G.me.translate(3, -1);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[36] += 25;
				attackScores[7] += 25;
				attackScores[23] += 25;
				attackScores[28] += 25;
				attackScores[12] += 25;
				attackScores[19] += 25;
				attackScores[35] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[36] += 25;
						attackScores[7] += 25;
						attackScores[23] += 25;
						attackScores[28] += 25;
						attackScores[12] += 25;
						attackScores[19] += 25;
						attackScores[35] += 25;
					} else {
						attackScores[36] += G.paintPerChips() * 500;
						attackScores[7] += G.paintPerChips() * 500;
						attackScores[23] += G.paintPerChips() * 500;
						attackScores[28] += G.paintPerChips() * 500;
						attackScores[12] += G.paintPerChips() * 500;
						attackScores[19] += G.paintPerChips() * 500;
						attackScores[35] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[36] += 25;
					attackScores[7] += 25;
					attackScores[23] += 25;
					attackScores[28] += 25;
					attackScores[12] += 25;
					attackScores[19] += 25;
					attackScores[35] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[23] += 50;
				attackScores[28] += 50;
				attackScores[12] += 50;
				attackScores[19] += 50;
				attackScores[35] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[36] += 50;
					attackScores[7] += 50;
					attackScores[23] += 50;
					attackScores[28] += 50;
					attackScores[12] += 50;
					attackScores[19] += 50;
					attackScores[35] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[36] += 50;
					attackScores[7] += 50;
					attackScores[23] += 50;
					attackScores[28] += 50;
					attackScores[12] += 50;
					attackScores[19] += 50;
					attackScores[35] += 50;
				}
			}
		}
		loc = G.me.translate(3, 1);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[8] += 25;
				attackScores[35] += 25;
				attackScores[12] += 25;
				attackScores[28] += 25;
				attackScores[24] += 25;
				attackScores[20] += 25;
				attackScores[36] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[8] += 25;
						attackScores[35] += 25;
						attackScores[12] += 25;
						attackScores[28] += 25;
						attackScores[24] += 25;
						attackScores[20] += 25;
						attackScores[36] += 25;
					} else {
						attackScores[8] += G.paintPerChips() * 500;
						attackScores[35] += G.paintPerChips() * 500;
						attackScores[12] += G.paintPerChips() * 500;
						attackScores[28] += G.paintPerChips() * 500;
						attackScores[24] += G.paintPerChips() * 500;
						attackScores[20] += G.paintPerChips() * 500;
						attackScores[36] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[8] += 25;
					attackScores[35] += 25;
					attackScores[12] += 25;
					attackScores[28] += 25;
					attackScores[24] += 25;
					attackScores[20] += 25;
					attackScores[36] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[12] += 50;
				attackScores[28] += 50;
				attackScores[24] += 50;
				attackScores[20] += 50;
				attackScores[36] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[8] += 50;
					attackScores[35] += 50;
					attackScores[12] += 50;
					attackScores[28] += 50;
					attackScores[24] += 50;
					attackScores[20] += 50;
					attackScores[36] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[8] += 50;
					attackScores[35] += 50;
					attackScores[12] += 50;
					attackScores[28] += 50;
					attackScores[24] += 50;
					attackScores[20] += 50;
					attackScores[36] += 50;
				}
			}
		}
		loc = G.me.translate(-3, -2);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[15] += 25;
				attackScores[25] += 25;
				attackScores[21] += 25;
				attackScores[13] += 25;
				attackScores[29] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[15] += 25;
						attackScores[25] += 25;
						attackScores[21] += 25;
						attackScores[13] += 25;
						attackScores[29] += 25;
					} else {
						attackScores[15] += G.paintPerChips() * 500;
						attackScores[25] += G.paintPerChips() * 500;
						attackScores[21] += G.paintPerChips() * 500;
						attackScores[13] += G.paintPerChips() * 500;
						attackScores[29] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[15] += 25;
					attackScores[25] += 25;
					attackScores[21] += 25;
					attackScores[13] += 25;
					attackScores[29] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[21] += 50;
				attackScores[13] += 50;
				attackScores[29] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[15] += 50;
					attackScores[25] += 50;
					attackScores[21] += 50;
					attackScores[13] += 50;
					attackScores[29] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[15] += 50;
					attackScores[25] += 50;
					attackScores[21] += 50;
					attackScores[13] += 50;
					attackScores[29] += 50;
				}
			}
		}
		loc = G.me.translate(-3, 2);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[16] += 25;
				attackScores[25] += 25;
				attackScores[30] += 25;
				attackScores[14] += 25;
				attackScores[22] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[16] += 25;
						attackScores[25] += 25;
						attackScores[30] += 25;
						attackScores[14] += 25;
						attackScores[22] += 25;
					} else {
						attackScores[16] += G.paintPerChips() * 500;
						attackScores[25] += G.paintPerChips() * 500;
						attackScores[30] += G.paintPerChips() * 500;
						attackScores[14] += G.paintPerChips() * 500;
						attackScores[22] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[16] += 25;
					attackScores[25] += 25;
					attackScores[30] += 25;
					attackScores[14] += 25;
					attackScores[22] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[30] += 50;
				attackScores[14] += 50;
				attackScores[22] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[16] += 50;
					attackScores[25] += 50;
					attackScores[30] += 50;
					attackScores[14] += 50;
					attackScores[22] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[16] += 50;
					attackScores[25] += 50;
					attackScores[30] += 50;
					attackScores[14] += 50;
					attackScores[22] += 50;
				}
			}
		}
		loc = G.me.translate(-2, -3);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[26] += 25;
				attackScores[13] += 25;
				attackScores[31] += 25;
				attackScores[15] += 25;
				attackScores[21] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[26] += 25;
						attackScores[13] += 25;
						attackScores[31] += 25;
						attackScores[15] += 25;
						attackScores[21] += 25;
					} else {
						attackScores[26] += G.paintPerChips() * 500;
						attackScores[13] += G.paintPerChips() * 500;
						attackScores[31] += G.paintPerChips() * 500;
						attackScores[15] += G.paintPerChips() * 500;
						attackScores[21] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[26] += 25;
					attackScores[13] += 25;
					attackScores[31] += 25;
					attackScores[15] += 25;
					attackScores[21] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[31] += 50;
				attackScores[15] += 50;
				attackScores[21] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[26] += 50;
					attackScores[13] += 50;
					attackScores[31] += 50;
					attackScores[15] += 50;
					attackScores[21] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[26] += 50;
					attackScores[13] += 50;
					attackScores[31] += 50;
					attackScores[15] += 50;
					attackScores[21] += 50;
				}
			}
		}
		loc = G.me.translate(-2, 3);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[27] += 25;
				attackScores[14] += 25;
				attackScores[22] += 25;
				attackScores[16] += 25;
				attackScores[32] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[27] += 25;
						attackScores[14] += 25;
						attackScores[22] += 25;
						attackScores[16] += 25;
						attackScores[32] += 25;
					} else {
						attackScores[27] += G.paintPerChips() * 500;
						attackScores[14] += G.paintPerChips() * 500;
						attackScores[22] += G.paintPerChips() * 500;
						attackScores[16] += G.paintPerChips() * 500;
						attackScores[32] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[27] += 25;
					attackScores[14] += 25;
					attackScores[22] += 25;
					attackScores[16] += 25;
					attackScores[32] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[22] += 50;
				attackScores[16] += 50;
				attackScores[32] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[27] += 50;
					attackScores[14] += 50;
					attackScores[22] += 50;
					attackScores[16] += 50;
					attackScores[32] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[27] += 50;
					attackScores[14] += 50;
					attackScores[22] += 50;
					attackScores[16] += 50;
					attackScores[32] += 50;
				}
			}
		}
		loc = G.me.translate(2, -3);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[19] += 25;
				attackScores[26] += 25;
				attackScores[23] += 25;
				attackScores[17] += 25;
				attackScores[33] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[19] += 25;
						attackScores[26] += 25;
						attackScores[23] += 25;
						attackScores[17] += 25;
						attackScores[33] += 25;
					} else {
						attackScores[19] += G.paintPerChips() * 500;
						attackScores[26] += G.paintPerChips() * 500;
						attackScores[23] += G.paintPerChips() * 500;
						attackScores[17] += G.paintPerChips() * 500;
						attackScores[33] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[19] += 25;
					attackScores[26] += 25;
					attackScores[23] += 25;
					attackScores[17] += 25;
					attackScores[33] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[23] += 50;
				attackScores[17] += 50;
				attackScores[33] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[19] += 50;
					attackScores[26] += 50;
					attackScores[23] += 50;
					attackScores[17] += 50;
					attackScores[33] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[19] += 50;
					attackScores[26] += 50;
					attackScores[23] += 50;
					attackScores[17] += 50;
					attackScores[33] += 50;
				}
			}
		}
		loc = G.me.translate(2, 3);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[27] += 25;
				attackScores[20] += 25;
				attackScores[18] += 25;
				attackScores[24] += 25;
				attackScores[34] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[27] += 25;
						attackScores[20] += 25;
						attackScores[18] += 25;
						attackScores[24] += 25;
						attackScores[34] += 25;
					} else {
						attackScores[27] += G.paintPerChips() * 500;
						attackScores[20] += G.paintPerChips() * 500;
						attackScores[18] += G.paintPerChips() * 500;
						attackScores[24] += G.paintPerChips() * 500;
						attackScores[34] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[27] += 25;
					attackScores[20] += 25;
					attackScores[18] += 25;
					attackScores[24] += 25;
					attackScores[34] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[18] += 50;
				attackScores[24] += 50;
				attackScores[34] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[27] += 50;
					attackScores[20] += 50;
					attackScores[18] += 50;
					attackScores[24] += 50;
					attackScores[34] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[27] += 50;
					attackScores[20] += 50;
					attackScores[18] += 50;
					attackScores[24] += 50;
					attackScores[34] += 50;
				}
			}
		}
		loc = G.me.translate(3, -2);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[28] += 25;
				attackScores[17] += 25;
				attackScores[35] += 25;
				attackScores[19] += 25;
				attackScores[23] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[28] += 25;
						attackScores[17] += 25;
						attackScores[35] += 25;
						attackScores[19] += 25;
						attackScores[23] += 25;
					} else {
						attackScores[28] += G.paintPerChips() * 500;
						attackScores[17] += G.paintPerChips() * 500;
						attackScores[35] += G.paintPerChips() * 500;
						attackScores[19] += G.paintPerChips() * 500;
						attackScores[23] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[28] += 25;
					attackScores[17] += 25;
					attackScores[35] += 25;
					attackScores[19] += 25;
					attackScores[23] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[35] += 50;
				attackScores[19] += 50;
				attackScores[23] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[28] += 50;
					attackScores[17] += 50;
					attackScores[35] += 50;
					attackScores[19] += 50;
					attackScores[23] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[28] += 50;
					attackScores[17] += 50;
					attackScores[35] += 50;
					attackScores[19] += 50;
					attackScores[23] += 50;
				}
			}
		}
		loc = G.me.translate(3, 2);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[18] += 25;
				attackScores[28] += 25;
				attackScores[20] += 25;
				attackScores[36] += 25;
				attackScores[24] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[18] += 25;
						attackScores[28] += 25;
						attackScores[20] += 25;
						attackScores[36] += 25;
						attackScores[24] += 25;
					} else {
						attackScores[18] += G.paintPerChips() * 500;
						attackScores[28] += G.paintPerChips() * 500;
						attackScores[20] += G.paintPerChips() * 500;
						attackScores[36] += G.paintPerChips() * 500;
						attackScores[24] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[18] += 25;
					attackScores[28] += 25;
					attackScores[20] += 25;
					attackScores[36] += 25;
					attackScores[24] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[20] += 50;
				attackScores[36] += 50;
				attackScores[24] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[18] += 50;
					attackScores[28] += 50;
					attackScores[20] += 50;
					attackScores[36] += 50;
					attackScores[24] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[18] += 50;
					attackScores[28] += 50;
					attackScores[20] += 50;
					attackScores[36] += 50;
					attackScores[24] += 50;
				}
			}
		}
		loc = G.me.translate(-4, 0);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[9] += 25;
				attackScores[29] += 25;
				attackScores[25] += 25;
				attackScores[30] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[9] += 25;
						attackScores[29] += 25;
						attackScores[25] += 25;
						attackScores[30] += 25;
					} else {
						attackScores[9] += G.paintPerChips() * 500;
						attackScores[29] += G.paintPerChips() * 500;
						attackScores[25] += G.paintPerChips() * 500;
						attackScores[30] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[9] += 25;
					attackScores[29] += 25;
					attackScores[25] += 25;
					attackScores[30] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[29] += 50;
				attackScores[25] += 50;
				attackScores[30] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[9] += 50;
					attackScores[29] += 50;
					attackScores[25] += 50;
					attackScores[30] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[9] += 50;
					attackScores[29] += 50;
					attackScores[25] += 50;
					attackScores[30] += 50;
				}
			}
		}
		loc = G.me.translate(0, -4);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[10] += 25;
				attackScores[33] += 25;
				attackScores[26] += 25;
				attackScores[31] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[10] += 25;
						attackScores[33] += 25;
						attackScores[26] += 25;
						attackScores[31] += 25;
					} else {
						attackScores[10] += G.paintPerChips() * 500;
						attackScores[33] += G.paintPerChips() * 500;
						attackScores[26] += G.paintPerChips() * 500;
						attackScores[31] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[10] += 25;
					attackScores[33] += 25;
					attackScores[26] += 25;
					attackScores[31] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[33] += 50;
				attackScores[26] += 50;
				attackScores[31] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[10] += 50;
					attackScores[33] += 50;
					attackScores[26] += 50;
					attackScores[31] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[10] += 50;
					attackScores[33] += 50;
					attackScores[26] += 50;
					attackScores[31] += 50;
				}
			}
		}
		loc = G.me.translate(0, 4);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[11] += 25;
				attackScores[32] += 25;
				attackScores[27] += 25;
				attackScores[34] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[11] += 25;
						attackScores[32] += 25;
						attackScores[27] += 25;
						attackScores[34] += 25;
					} else {
						attackScores[11] += G.paintPerChips() * 500;
						attackScores[32] += G.paintPerChips() * 500;
						attackScores[27] += G.paintPerChips() * 500;
						attackScores[34] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[11] += 25;
					attackScores[32] += 25;
					attackScores[27] += 25;
					attackScores[34] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[32] += 50;
				attackScores[27] += 50;
				attackScores[34] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[11] += 50;
					attackScores[32] += 50;
					attackScores[27] += 50;
					attackScores[34] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[11] += 50;
					attackScores[32] += 50;
					attackScores[27] += 50;
					attackScores[34] += 50;
				}
			}
		}
		loc = G.me.translate(4, 0);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[12] += 25;
				attackScores[35] += 25;
				attackScores[36] += 25;
				attackScores[28] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[12] += 25;
						attackScores[35] += 25;
						attackScores[36] += 25;
						attackScores[28] += 25;
					} else {
						attackScores[12] += G.paintPerChips() * 500;
						attackScores[35] += G.paintPerChips() * 500;
						attackScores[36] += G.paintPerChips() * 500;
						attackScores[28] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[12] += 25;
					attackScores[35] += 25;
					attackScores[36] += 25;
					attackScores[28] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[35] += 50;
				attackScores[36] += 50;
				attackScores[28] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[12] += 50;
					attackScores[35] += 50;
					attackScores[36] += 50;
					attackScores[28] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[12] += 50;
					attackScores[35] += 50;
					attackScores[36] += 50;
					attackScores[28] += 50;
				}
			}
		}
		loc = G.me.translate(-4, -1);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[13] += 25;
				attackScores[29] += 25;
				attackScores[25] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[13] += 25;
						attackScores[29] += 25;
						attackScores[25] += 25;
					} else {
						attackScores[13] += G.paintPerChips() * 500;
						attackScores[29] += G.paintPerChips() * 500;
						attackScores[25] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[13] += 25;
					attackScores[29] += 25;
					attackScores[25] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[29] += 50;
				attackScores[25] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[13] += 50;
					attackScores[29] += 50;
					attackScores[25] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[13] += 50;
					attackScores[29] += 50;
					attackScores[25] += 50;
				}
			}
		}
		loc = G.me.translate(-4, 1);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[14] += 25;
				attackScores[25] += 25;
				attackScores[30] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[14] += 25;
						attackScores[25] += 25;
						attackScores[30] += 25;
					} else {
						attackScores[14] += G.paintPerChips() * 500;
						attackScores[25] += G.paintPerChips() * 500;
						attackScores[30] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[14] += 25;
					attackScores[25] += 25;
					attackScores[30] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[25] += 50;
				attackScores[30] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[14] += 50;
					attackScores[25] += 50;
					attackScores[30] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[14] += 50;
					attackScores[25] += 50;
					attackScores[30] += 50;
				}
			}
		}
		loc = G.me.translate(-1, -4);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[15] += 25;
				attackScores[26] += 25;
				attackScores[31] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[15] += 25;
						attackScores[26] += 25;
						attackScores[31] += 25;
					} else {
						attackScores[15] += G.paintPerChips() * 500;
						attackScores[26] += G.paintPerChips() * 500;
						attackScores[31] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[15] += 25;
					attackScores[26] += 25;
					attackScores[31] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[26] += 50;
				attackScores[31] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[15] += 50;
					attackScores[26] += 50;
					attackScores[31] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[15] += 50;
					attackScores[26] += 50;
					attackScores[31] += 50;
				}
			}
		}
		loc = G.me.translate(-1, 4);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[16] += 25;
				attackScores[32] += 25;
				attackScores[27] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[16] += 25;
						attackScores[32] += 25;
						attackScores[27] += 25;
					} else {
						attackScores[16] += G.paintPerChips() * 500;
						attackScores[32] += G.paintPerChips() * 500;
						attackScores[27] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[16] += 25;
					attackScores[32] += 25;
					attackScores[27] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[32] += 50;
				attackScores[27] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[16] += 50;
					attackScores[32] += 50;
					attackScores[27] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[16] += 50;
					attackScores[32] += 50;
					attackScores[27] += 50;
				}
			}
		}
		loc = G.me.translate(1, -4);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[17] += 25;
				attackScores[33] += 25;
				attackScores[26] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[17] += 25;
						attackScores[33] += 25;
						attackScores[26] += 25;
					} else {
						attackScores[17] += G.paintPerChips() * 500;
						attackScores[33] += G.paintPerChips() * 500;
						attackScores[26] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[17] += 25;
					attackScores[33] += 25;
					attackScores[26] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[33] += 50;
				attackScores[26] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[17] += 50;
					attackScores[33] += 50;
					attackScores[26] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[17] += 50;
					attackScores[33] += 50;
					attackScores[26] += 50;
				}
			}
		}
		loc = G.me.translate(1, 4);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[18] += 25;
				attackScores[27] += 25;
				attackScores[34] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[18] += 25;
						attackScores[27] += 25;
						attackScores[34] += 25;
					} else {
						attackScores[18] += G.paintPerChips() * 500;
						attackScores[27] += G.paintPerChips() * 500;
						attackScores[34] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[18] += 25;
					attackScores[27] += 25;
					attackScores[34] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[27] += 50;
				attackScores[34] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[18] += 50;
					attackScores[27] += 50;
					attackScores[34] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[18] += 50;
					attackScores[27] += 50;
					attackScores[34] += 50;
				}
			}
		}
		loc = G.me.translate(4, -1);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[19] += 25;
				attackScores[28] += 25;
				attackScores[35] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[19] += 25;
						attackScores[28] += 25;
						attackScores[35] += 25;
					} else {
						attackScores[19] += G.paintPerChips() * 500;
						attackScores[28] += G.paintPerChips() * 500;
						attackScores[35] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[19] += 25;
					attackScores[28] += 25;
					attackScores[35] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[28] += 50;
				attackScores[35] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[19] += 50;
					attackScores[28] += 50;
					attackScores[35] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[19] += 50;
					attackScores[28] += 50;
					attackScores[35] += 50;
				}
			}
		}
		loc = G.me.translate(4, 1);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[20] += 25;
				attackScores[28] += 25;
				attackScores[36] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[20] += 25;
						attackScores[28] += 25;
						attackScores[36] += 25;
					} else {
						attackScores[20] += G.paintPerChips() * 500;
						attackScores[28] += G.paintPerChips() * 500;
						attackScores[36] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[20] += 25;
					attackScores[28] += 25;
					attackScores[36] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[28] += 50;
				attackScores[36] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[20] += 50;
					attackScores[28] += 50;
					attackScores[36] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[20] += 50;
					attackScores[28] += 50;
					attackScores[36] += 50;
				}
			}
		}
		loc = G.me.translate(-3, -3);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[31] += 25;
				attackScores[29] += 25;
				attackScores[21] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[31] += 25;
						attackScores[29] += 25;
						attackScores[21] += 25;
					} else {
						attackScores[31] += G.paintPerChips() * 500;
						attackScores[29] += G.paintPerChips() * 500;
						attackScores[21] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[31] += 25;
					attackScores[29] += 25;
					attackScores[21] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[21] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[31] += 50;
					attackScores[29] += 50;
					attackScores[21] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[31] += 50;
					attackScores[29] += 50;
					attackScores[21] += 50;
				}
			}
		}
		loc = G.me.translate(-3, 3);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[32] += 25;
				attackScores[30] += 25;
				attackScores[22] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[32] += 25;
						attackScores[30] += 25;
						attackScores[22] += 25;
					} else {
						attackScores[32] += G.paintPerChips() * 500;
						attackScores[30] += G.paintPerChips() * 500;
						attackScores[22] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[32] += 25;
					attackScores[30] += 25;
					attackScores[22] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[22] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[32] += 50;
					attackScores[30] += 50;
					attackScores[22] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[32] += 50;
					attackScores[30] += 50;
					attackScores[22] += 50;
				}
			}
		}
		loc = G.me.translate(3, -3);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[35] += 25;
				attackScores[33] += 25;
				attackScores[23] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[35] += 25;
						attackScores[33] += 25;
						attackScores[23] += 25;
					} else {
						attackScores[35] += G.paintPerChips() * 500;
						attackScores[33] += G.paintPerChips() * 500;
						attackScores[23] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[35] += 25;
					attackScores[33] += 25;
					attackScores[23] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[23] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[35] += 50;
					attackScores[33] += 50;
					attackScores[23] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[35] += 50;
					attackScores[33] += 50;
					attackScores[23] += 50;
				}
			}
		}
		loc = G.me.translate(3, 3);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[34] += 25;
				attackScores[36] += 25;
				attackScores[24] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[34] += 25;
						attackScores[36] += 25;
						attackScores[24] += 25;
					} else {
						attackScores[34] += G.paintPerChips() * 500;
						attackScores[36] += G.paintPerChips() * 500;
						attackScores[24] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[34] += 25;
					attackScores[36] += 25;
					attackScores[24] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[24] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[34] += 50;
					attackScores[36] += 50;
					attackScores[24] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[34] += 50;
					attackScores[36] += 50;
					attackScores[24] += 50;
				}
			}
		}
		loc = G.me.translate(-4, -2);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[21] += 25;
				attackScores[29] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[21] += 25;
						attackScores[29] += 25;
					} else {
						attackScores[21] += G.paintPerChips() * 500;
						attackScores[29] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[21] += 25;
					attackScores[29] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[29] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[21] += 50;
					attackScores[29] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[21] += 50;
					attackScores[29] += 50;
				}
			}
		}
		loc = G.me.translate(-4, 2);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[22] += 25;
				attackScores[30] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[22] += 25;
						attackScores[30] += 25;
					} else {
						attackScores[22] += G.paintPerChips() * 500;
						attackScores[30] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[22] += 25;
					attackScores[30] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[30] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[22] += 50;
					attackScores[30] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[22] += 50;
					attackScores[30] += 50;
				}
			}
		}
		loc = G.me.translate(-2, -4);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[21] += 25;
				attackScores[31] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[21] += 25;
						attackScores[31] += 25;
					} else {
						attackScores[21] += G.paintPerChips() * 500;
						attackScores[31] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[21] += 25;
					attackScores[31] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[31] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[21] += 50;
					attackScores[31] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[21] += 50;
					attackScores[31] += 50;
				}
			}
		}
		loc = G.me.translate(-2, 4);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[22] += 25;
				attackScores[32] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[22] += 25;
						attackScores[32] += 25;
					} else {
						attackScores[22] += G.paintPerChips() * 500;
						attackScores[32] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[22] += 25;
					attackScores[32] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[32] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[22] += 50;
					attackScores[32] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[22] += 50;
					attackScores[32] += 50;
				}
			}
		}
		loc = G.me.translate(2, -4);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[23] += 25;
				attackScores[33] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[23] += 25;
						attackScores[33] += 25;
					} else {
						attackScores[23] += G.paintPerChips() * 500;
						attackScores[33] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[23] += 25;
					attackScores[33] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[33] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[23] += 50;
					attackScores[33] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[23] += 50;
					attackScores[33] += 50;
				}
			}
		}
		loc = G.me.translate(2, 4);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[24] += 25;
				attackScores[34] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[24] += 25;
						attackScores[34] += 25;
					} else {
						attackScores[24] += G.paintPerChips() * 500;
						attackScores[34] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[24] += 25;
					attackScores[34] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[34] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[24] += 50;
					attackScores[34] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[24] += 50;
					attackScores[34] += 50;
				}
			}
		}
		loc = G.me.translate(4, -2);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[23] += 25;
				attackScores[35] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[23] += 25;
						attackScores[35] += 25;
					} else {
						attackScores[23] += G.paintPerChips() * 500;
						attackScores[35] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[23] += 25;
					attackScores[35] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[35] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[23] += 50;
					attackScores[35] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[23] += 50;
					attackScores[35] += 50;
				}
			}
		}
		loc = G.me.translate(4, 2);
		if (G.rc.onTheMap(loc)) {
			PaintType paint = G.rc.senseMapInfo(loc).getPaint();
			if (paint == PaintType.EMPTY) {
				attackScores[24] += 25;
				attackScores[36] += 25;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					if (ruinsString.indexOf(loc.toString()) == -1) {
						attackScores[24] += 25;
						attackScores[36] += 25;
					} else {
						attackScores[24] += G.paintPerChips() * 500;
						attackScores[36] += G.paintPerChips() * 500;
					}
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[24] += 25;
					attackScores[36] += 25;
				}
			}
			else if (paint.isEnemy()) {
				attackScores[36] += 50;
				if (G.opponentRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[24] += 50;
					attackScores[36] += 50;
				} else if (G.allyRobotsString.indexOf(loc.toString()) != -1) {
					attackScores[24] += 50;
					attackScores[36] += 50;
				}
			}
		}
    }

    public static void exploreMoveScores() throws Exception {
        moveScores = Motion.defaultMicro.micro(Motion.bug2Helper(G.me, Motion.exploreRandomlyLoc(), Motion.TOWARDS, 0, 0), Motion.exploreLoc);
    }

    public static void attackAttackScores() throws Exception {
        exploreAttackScores1();
        exploreAttackScores2();
    }

    public static void attackMoveScores() throws Exception {
        moveScores = Motion.defaultMicro.micro(Motion.bug2Helper(G.me, attackTarget, Motion.TOWARDS, 0, 0), attackTarget);
    }

    /**
     * Searches for towers/ruins in POI to attack, and enters ATTACK mode if found
     * target
     */
    public static void updateAttackTarget() throws Exception {
        if (attackTargetTower != -1) {
            if (POI.towerTeams[attackTargetTower] != G.opponentTeam) {
                attackTarget = G.invalidLoc;
            } else {
                MapLocation loc = POI.towerLocs[attackTargetTower];
                search: if (G.me.distanceSquaredTo(loc) <= 4) {
                    for (int y = -2; y <= 2; y++) {
                        for (int x = -2; x <= 2; x++) {
                            if (G.rc.senseMapInfo(loc.translate(x, y)).getPaint().isEnemy()) {
                                break search;
                            }
                        }
                    }
                    attackTarget = G.invalidLoc;
                }
            }
            // TODO: make it change targets if it finds ruin with 24 empty/ally paint
            // TODO: STOP CIRCLING AROUND OUR OWN TOWERS ALREADY
        }
        if (attackTarget.x == -1) {
            int best = -1;
            int bestWeight = 0;
            for (int i = POI.numberOfTowers; --i >= 0;) {
                if (POI.towerTeams[i] == G.team) {
                    continue;
                }
                if (POI.towerTeams[i] == Team.NEUTRAL && G.rc.getNumberTowers() == 25) {
                    continue;
                }
                MapLocation loc = POI.towerLocs[i];
                int distance = Motion.getChebyshevDistance(G.me, loc);
                int weight = -distance;
                if (triedAttackTargets.indexOf("" + (char) i) != -1) {
                // if (!(G.round <= VISIT_TIMEOUT || G.getLastVisited(loc.x, loc.y) + VISIT_TIMEOUT < G.round)) {
                    weight -= 1000;
                    if (POI.towerTeams[i] == Team.NEUTRAL) {
                        continue;
                    }
                }
                if (POI.towerTeams[i] == G.opponentTeam) {
                    weight += 100;
                }
                if (best == -1 || weight > bestWeight) {
                    best = i;
                    bestWeight = weight;
                }
            }
            if (best == -1) {
                mode = EXPLORE;
                return;
            }
            attackTargetTower = best;
            attackTarget = POI.towerLocs[best];
            triedAttackTargets.append((char) best);
            // G.setLastVisited(attackTarget, G.round);
            mode = ATTACK;
        }
    }
}
