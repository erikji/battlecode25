...
public class Tower {
    ...
    public static MapLocation[] prioritySpawnLocs;
    public static MapLocation[] spawnLocs;
    public static MapLocation[] centerSpawnLocs;
    public static int level;

    public static void init() throws Exception {
        prioritySpawnLocs = new MapLocation[] {
        ...
        centerSpawnLocs = new MapLocation[] {
            G.me.add(Direction.NORTH),
            G.me.add(Direction.NORTH).add(Direction.NORTH),
            G.me.add(Direction.NORTHEAST),
            G.me.add(Direction.EAST),
    ...
    public static void spawnBot(UnitType t, double soldierWeight, double splasherWeight, double mopperWeight) throws Exception {
        int offset = Random.rand() % 8;
        switch (t) {
            case UnitType.MOPPER:
                for (int i = 8; --i >= 0;) {
                    MapLocation loc = prioritySpawnLocs[(i + offset) % 8];
                ...
                for (int i = 4; --i >= 0;) {
                    MapLocation loc = spawnLocs[(i + offset) % 4];
                    if (G.rc.canBuildRobot(UnitType.MOPPER, loc)) {
                        G.rc.buildRobot(UnitType.MOPPER, loc);
                        spawnedRobots++;
                        spawnedMoppers++;
            ...
            case UnitType.SPLASHER:
                for (MapLocation loc : centerSpawnLocs) {
    ...
    public static void run() throws Exception {
        ...
        if (G.rc.getNumberTowers() == 25 || G.rc.getMoney() - trying.moneyCost >= 900) {
            spawnBot(trying, soldierWeight, splasherWeight, mopperWeight);
        }
