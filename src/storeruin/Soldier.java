...
public class Soldier {
    ...
    public static MapLocation lastRuinLocation = null; // Stores ruin location to return to if we retreat
    ...
    public static void explore() throws Exception {
        ...
        // TOWER_CEIL encourages building SRPs to help build more towers
        if (lastRuinLocation != null) {
            // should be fine because only used here
            if (G.rc.canSenseLocation(lastRuinLocation)) {
                lastRuinLocation = null;
            }
        }
        if (lastRuinLocation != null) {
            exploreLocation = lastRuinLocation;
        }
        else if (G.round > TOWER_CEIL_ROUND || G.rc.getNumberTowers() <= TOWER_CEIL) {
